...
namespace JIT.CPOS.Web.Interface.Data
{
    public partial class OrderData : System.Web.UI.Page
    {
        ...
        public string SetPayOrder(string pBillCreateIP)
        {
            ...
            try
            {
                string reqContent = Request["ReqContent"].ToString().Trim(); 
                ...
                decimal appOrderAmount = new OnlineShoppingItemBLL(loggingSessionInfo).GetOrderAmmount(reqObj.special.orderId);
                  ...
                  if (appOrderAmount == 0)//金额为零不能进行支付，这个是卡升级的，用的是传过来的值
                {
                    appOrderAmount = Convert.ToDecimal(reqObj.special.amount);
                }
                else { //普通订单的情况
                    decimal useBalance = new OnlineShoppingItemBLL(loggingSessionInfo).GetUseBalance(reqObj.special.orderId);
                    //减去余额
                    appOrderAmount = appOrderAmount - useBalance;          
                }
                ...
                else
                {
                    ...
                    if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        if (appInterfaceInfo != null)
                        {
                            ...
                            var wxUserInfo = new VipBLL(loggingSessionInfo).GetByID(reqObj.common.userId);
                            if (wxUserInfo != null)
                            {

...
function LinkAddCustomerId(LinkUrl) {
    ...
    if (LinkUrl.indexOf("customerId") <= -1) {
        if (LinkUrl.indexOf("?") > -1) {
            location.href = LinkUrl + "&customerId=" + getParam("customerId") +  "&storeId=" + getParam("storeId") +"&startTime=" + getParam("startTime") +"&endTime=" + getParam("endTime") +"&random=" + Math.floor(Math.random() * 10000000);
        } else {
            location.href =LinkUrl + "?customerId=" + getParam("customerId") +  "&storeId=" + getParam("storeId")  +"&startTime=" + getParam("startTime") +"&endTime=" + getParam("endTime") +"&random=" + Math.floor(Math.random() * 10000000);
        }
    ...
    $.ajax = function(params) {
        ...
        if (params['dataType'] != "html" && typeof params['data'] != 'undefined') {
            if (params['data'] != null) {
                ...
                data.plat = "pc";
				var getReqContent =JSON.parse(data.ReqContent);
				getReqContent.special.beginDate=getParam("startTime");
				getReqContent.special.endDate=getParam("endTime");
				data.ReqContent  = JSON.stringify(getReqContent);
                params['data'] = data;
            }
        }
        pre_ajax(params);
    ...
    function() {
        ...
        if (getHref.indexOf("customerId") <= -1 && getHref != 'javascript:void(0)' && getHref != 'javascript:;' && getHref.indexOf("tel:") <= -1 && getHref.indexOf("mailto:") <= -1) {
            if (getHref.indexOf("?") > -1) {
                location.href =getHref + "&customerId=" + getParam("customerId") +  "&storeId=" + getParam("storeId") +  "&startTime=" + getParam("startTime") +"&endTime=" + getParam("endTime") +"&random=" + Math.floor(Math.random() * 10000000);
            } else {
             location.href =getHref + "?customerId=" + getParam("customerId") +  "&storeId=" + getParam("storeId") +  "&startTime=" + getParam("startTime") +"&endTime=" + getParam("endTime") +"&random=" + Math.floor(Math.random() * 10000000);
            }
        }
...
function Init() {
    if ($("#MenuBox").length > 0) {
        ...
        IMenu += '><div class="smain1"><img src="images/nt1.png"></div>返回</a></li> <li><a href="index.html"><div class="smain1"><img src="images/nt2.png"></div> 首页</a></li><li><a onclick="shareAP(\'share.html\')" href="javascript:void(0)"';
        if (getRel == "3") {
            IMenu += " class='ImenuClass' "
        }

Jit.AM.defindPage({
	...
	addPageEvent:function(){
		...
		$("#section").delegate(".subBtn","tap",function(e){
			...
			if($this.hasClass('on')){
				...
				$('#totalQty').html(self.newTotalQty);
				$this.removeClass('on');
		...
		me.ajax({
			...
			success:function(data){
				...
				if(data.code == 200){
					...
					me.newTotalQty = me.initTotalQty = data.content.totalQty;
					
					$('#totalprice').html('￥'+data.content.totalAmount);
					$('#totalQty').html(me.newTotalQty);
	...
	allselect:function(){
		...
		if($allselect.hasClass('on')){
			...
			self.newTotalQty = self.initTotalQty;
			$allselect.addClass('on');
			$select.addClass('on');
			
			$('#totalprice').html(this.initTotalprice);
			$('#totalQty').html(this.newTotalQty);
		}
	},

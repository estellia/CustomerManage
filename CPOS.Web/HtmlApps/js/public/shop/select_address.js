Jit.AM.defindPage({
            ...
            success: function (data) {
                if (data.code == 200) {
                    ...
                    if (list.length <= 0) {
                        ...
                        html = '<div style="text-align:center;line-height:60px;">暂无添加配送地址！</div>';
    ...
    initEvent: function () {
		...
		$('#address_list').delegate("[name=address_item]","click",function(evt){
			var $this = $(this);
            var addressId = $this.attr('adrId');
            me.selectAdrId = addressId;

            $('[name=address_area]').removeClass('cur');

            $this.parents('.address_store').addClass('cur');
			me.setDefault(addressId);
			if(Jit.AM.getUrlParam('IsAddr')!=1){
				me.saveAddress();
			}

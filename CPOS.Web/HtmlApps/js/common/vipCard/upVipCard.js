Jit.AM.defindPage({
    ...
    initPage: function () {
        //确认领取
        this.elems.sureGet=$("#getCard .receiveBtn");
    ...
    initEvent:function(){
        ...
        //点击领取事件
        this.elems.sureGet.bind('click',function(){
			//是否有付费的卡0=没有；1=有
			if(that.isCostCardType){
				Jit.AM.toPage('VirtualGoodsList');
			}else{
    ...
    checkIsRegister:function(){
        ...
        me.ajax({
            ...
            success: function (data) {
				...
				me.isCostCardType = data.Data.MemberInfo.IsCostCardType;
				me.cardTypePrice = data.Data.MemberInfo.CardTypePrice;
				me.IsDealer = data.Data.MemberInfo.IsDealer;//是否是经销商 0=不是；1=是
				
				if(!!data.Data.MemberInfo.CardTypeImageUrl){
					me.cardTypePrice = ++me.cardTypePrice;
				}
				
				$('.vipUpgrade a').attr('href','javascript:Jit.AM.toPage("VirtualGoodsList","cardPrice='+me.cardTypePrice+'")');//(++me.cardTypePrice)
				if(!!me.IsDealer){
					$('.dealerPlan a').attr('href','javascript:Jit.AM.toPage("DealersPlanIndex")');
				}
				
				Jit.AM.setPageParam('isUpVipCardStatus',data.Data.MemberInfo.Status);
				//me.orderCount();
                //用户不存在,显示领卡界面
                if(data.ResultCode == 330 || (data.Data && data.Data.MemberInfo && data.Data.MemberInfo.CardTypeImageUrl=='')){
					//用户未注册
					/*
					Jit.UI.Prompt({
							'title':'亲，你还没有注册哦！',
							'des':'您现在可以：',
							'html':'<a href=javascript:Jit.AM.toPage("VipRegister"); class="btn01" style="width:70%;">无卡，注册</a>'});
					*/	
					
					
					
					
					$('#getVipCardPage .infoWrap').hide();
                ...
                }else if(data.ResultCode == 0 && data.Data && !!data.Data.MemberInfo.CardTypeImageUrl){
					//用户已注册
					//更新会员卡中心用户信息
					$('#getCard').hide();
                    //me.updateVipCardPageInfo(data.Data.MemberInfo);
	...
	orderCount:function(){
		...
		that.ajax({
	            ...
	            success: function(data) {
	                if(data.IsSuccess){
	                	var result = data.Data.GroupingOrderCounts,
							unpayCount = result[0].OrderCount,
							undispatchCount = result[1].OrderCount;
						if(unpayCount==0){
							$('.paymentStatus span').remove();
						}else{
							$('.paymentStatus span').html(unpayCount); //未付款
						}
						if(undispatchCount==0){
							$('.dispatchStatus span').remove();
						}else{
							$('.dispatchStatus span').html(undispatchCount); //待收货
						}
    ...
    updateVipCardPageInfo: function(info){
        ...
        var vipCardImg= info.CardTypeImageUrl || "../../../images/common/vipCard/defaultCard.png";
        $('[name=vipCardImg]').attr('src',vipCardImg);//会员卡图片
		//$('[info=vip_head]').attr('src',info.ImageUrl);//头像
        $('[info=vip_phone]').html(info.VipNo||'卡号未知'); //卡的编号
        $('[info=vip_balance]').html(info.Balance+'.00');//余额
        $('[info=vip_coupon]').html(info.CouponsCount);//优惠券
		$('[info=vip_returnAmount]').html(info.ReturnAmount);//返利
        $('[info=vip_integration]').html(info.Integration);//积分
        $('[info=vip_level]').html(info.VipLevelName?info.VipLevelName:"注册会员");//卡的类型
    },

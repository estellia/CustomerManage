Jit.AM.defindPage({
	...
	onPageLoad : function() {
		...
		this.objectId = JitPage.getUrlParam('eventId');
		if (!this.objectId) {
			this.alert("未获取到eventId");
		}
	...
	//我要加盟事件
	join : function() {
		if(!this.canUse){  //false为未获得注册表单
			this.alert("未获得注册表单!请联系管理员!");
		}else{
			$("#mask").show();
			this.showForm();
		}
	...
	loadData : function() {
		...
		this.ajax({
            ...
            data: {
                ...
                'eventId': this.objectId
            },
            success: function(data) {
                if (data && data.code == 200) {
                   $("#content").html(data.content.description);
                }
            }
        });
	...
	showForm : function() {
		...
		this.elems.registerFormArea.bind("webkitAnimationEnd", function() {//动画结束时事件
			...
			self.elems.registerFormArea.unbind("webkitAnimationEnd");
		});
	},
	...
	buildRegisterForm : function(items) {
		...
		for (var i in items) {
			...
			htmlstr += bd.template('tpl_block_item', items[i]);
		}

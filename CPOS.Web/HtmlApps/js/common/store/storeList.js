Jit.AM.defindPage({
	...
	loadPageData:function(callback){
		var self = this;
		var isAll=Jit.AM.getUrlParam("applicationId");
		//ajax获取顶部显示状态
	...
	filterData:function(){
		//清除老客户的缓存
		JitPage.setParams("storeListPageDatas",null);
		var self = this;
		if($("#citySelect option:selected").attr('value')=="-00"){
			if(!self.coords){
				self.geoLocation(function(position){
					/*	self.coords = position.coords;     //谷歌
					 self.SearchStoreList(self.coords.longitude+","+self.coords.latitude);*/
					self.coords=position.point;
					self.SearchStoreList(self.coords.lng+','+self.coords.lat);
				});
			}else{
				if(self.coords.longitude&&self.coords.latitude){
					self.coords.lng=self.coords.longitude;
					self.coords.lat=self.coords.latitude;
				}
				self.SearchStoreList(self.coords.lng+","+self.coords.lat);
				/*self.SearchStoreList(self.coords.longitude+","+self.coords.latitude);*/
			}
	...
	geoLocation:function(callback) {
		...
		geolocation.getCurrentPosition(function (position) {
			if (this.getStatus() == BMAP_STATUS_SUCCESS) {
				...
				self.showAlert("非常抱歉，我们暂时无法通过浏览器获取您的位置信息");

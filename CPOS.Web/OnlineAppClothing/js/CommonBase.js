...
function LoginExit(){
	...
	var ood = JSON.stringify({"userId":G("userId"),"openId":G("openId"),"customerId":G("customerId"),"islogin":G("islogin")});
	window.jsObj.indexc(ood);	
	LinkAddCustomerId("login.html");
	}else{
		LinkAddCustomerId("index.html");
	}
}
function andriodAlert(text){
	 if(getParam("plat") =="android"){
                 		 window.jsobj.Tips(text);
					 }else{
						 alert(text);
			}
	}
        ...
        $.ajax({
            ...
            success: function(o) {
				...
				if(getParam("plat") =="android"){
						...
						window.jsObj.indexc(ood);	
						if(getParam("back") =="camera"){
						window.jsObj.ToCamera();
							return false; 
						}
					
					}else{
				 		$("#isLoadComplete").append('<iframe scrolling="no" src="http://gotoapp?link=loginComplate" frameborder="0"></iframe>');
					 }
				 if(getParam("noAlert")!="1"){
                 	 alert("登录成功");
					if(getParam("vipCenter") == "1"){
...
function Init() {
    if ($("#MenuBox").length > 0) {
        ...
        var IMenu = '<ul><li><a onclick="history.back()" href="javascript:void(0)"';
        if (getRel == "1") {
...
var WebViewAppUI = {
		common:function(commonObj){						//{"locale":"zh","userId":"4e3a255e0f953027a63f796332cb0a7b","openId":"4e3a255e0f953027a63f796332cb0a7b","customerId":"f092ade7784547ff82edafbde9f291d6",islogin:"1"}
			var customerId = commonObj.customerId, abc = [];
			var getCookieBase = WeiXin.GetCookie("base");
			if(getCookieBase !=""){
			var CookieBase= JSON.parse(getCookieBase);
			if(CookieBase.length > 0 ){
				for(var i=0; i< CookieBase.length; i++){
					if(customerId == CookieBase[i].customerId){	
					abc.push(commonObj);
						}else{

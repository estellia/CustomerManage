...
var Vip = {
            ...
            success: function(o) {
                ...
                if (o.code == "200") {
					if(sd!="list"){
						...
						$("#isLoadComplete").append('<iframe scrolling="no" src="http://gotoapp?link=HasJoined" frameborder="0"></iframe>');
					}
                } else {
                    andriodAlert(o.description);
                }
	...
	setUpdateOrderDelivery:function(){
        ...
        if (deliveryId == "2") {
            if (idg == false) {
                andriodAlert("请选择提货的门店！");
                return false;
            }
            if (deliveryTime == "") {
                andriodAlert("请填写提货的时间！");
                return false;
            }

        }
        if (email != "" && !IsEmail(email)) {
            andriodAlert("请正确输入您的邮箱！");
            return false;
        }
        if (mobile == "") {
            andriodAlert("请输入必填项!");
...
function slVipTip(totalAmount){
			if(AppSet.IsLogin() > 0){
							...
							$("#LoginTip").html('<a href="login.html" class="jieSuanBtn" style="margin-right:10px; background:#a73856;width:100px;">登录/注册</a>本次消费可累计积分<span style="font-size:16px; color:#e84d93;" id="fenshu">'+Math.floor(totalAmount)+'</span>分，<br>快速加入会员，获取本次积分。');
						}
}
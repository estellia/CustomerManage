...
var Templates ={
		EventDetailTemp:function(ConnextObj){
				var inAppInput = '';
					 inAppInput = '<input type="button"  class="btnStylecc" value="报名" onclick="Event.ActionApply()" />';		
				var mapBtnString = '';
				if(ConnextObj.longitude != "" &&  ConnextObj.latitude!=""){
						mapBtnString = '<a href="javascript:OpenGoogleMap('+ConnextObj.longitude+','+ConnextObj.latitude+')">查看地图</a>';
					}
				
				var iString = '<section class="AppBanner" style="text-align:center; overflow:hidden; position:relative;"><img src="'+ConnextObj.imageUrl+'" width="100%"  /><div class="WAppBannerTit">已报名：'+ConnextObj.applyCount+'<br>已签到: '+ConnextObj.checkinCount+'<br>'+inAppInput+'</div></section><section class="DetailAll"><h2>'+ConnextObj.title+'</h2><div class="DetailLi"><h3><span>时间</span></h3><p>'+ConnextObj.timeStr+'</p></div><div class="DetailLi"><h3><span>地点</span></h3><p>'+ConnextObj.address+'&nbsp;&nbsp;'+mapBtnString+'</p></div><div class="DetailLi"><h3><span>活动发起</span></h3><p>'+ConnextObj.organizer+'</p></div><div class="DetailLi"><h3><span>联系人</span></h3><p>'+ConnextObj.contact+'</p></div><div class="DetailLi"><h3><span>介绍</span></h3><p>'+ConnextObj.description.replace(/\n/gi,"<br />")+'</p></div></section>';
				return iString; 
			},
        EventDetailApply: function (ConnextObj, param) {
			...
			var istring = '<div class="TipApply"></div><div id="AllQuestionId">'+QuestionString+'</div><div style="line-height:46px;" class="inputAndSelect"><input type="button" class="inputsubmitStyle " value="提交" onClick="Event.ApplySubmit(\''+param+'\')" /></div>'; 

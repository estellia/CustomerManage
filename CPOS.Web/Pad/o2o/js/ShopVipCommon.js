...
var Vip = {
    ...
    getShoppingCart: function(isfirst) {
        ...
        if (isfirst == "1") {
			var ctpl = _.template($("#getShoppingCartTemp").html()); 
			$("#commonDiv").html(ctpl)
            $("#VipCart").attr("page", "1");
        }
        var itemName = '';
        ...
        $.ajax({
            ...
            success: function(o) {
                ...
                if (isfirst == "1") {
					 o.content.isfirst ="1";
                    Win.Loading("CLOSE");
					$(".JianShu").html(o.content.totalQty);
                }
                ...
                if (o.code == "200") {
                    ...
                    if (Listdata.length > 0) {
						 o.content.odata ="1";
                        var tpl = _.template($("#getShoppingCartList").html(), o.content);
                        if (isfirst != "1") {
                            $("#VipCart").append(tpl);
	 ...
	 getItemKeep: function(isfirst) {
        ...
        if (isfirst == "1") {
            $("#ItemKeep").attr("page", "1");
        }
        var itemName = '';
        $("#ItemKeep").attr("isloading", "1");
        var page = $("#ItemKeep").attr("page");
        var jsonarr = {
            'action': "getItemKeep",
            ReqContent: JSON.stringify({
                "common": Base.All(),
        ...
        $.ajax({
            ...
            success: function(o) {
                ...
                $("#ItemKeep").attr("isloading", "0");
				 o.content.isfirst = isfirst;
				
                if (isfirst == "1") {
                    Win.Loading("CLOSE");
                }
                Win.Loading("CLOSE", "#ItemKeep");
                if (o.code == "200") {
                    $("#ItemKeep").attr("page", parseInt(page) + 1).attr("isnext", o.content.isNext);
                    var Listdata = o.content.itemList;
					
					 $(".JianShu").text(o.content.itemList.length);
                    if (Listdata.length > 0) {
						 o.content.odata = "1";
    ...
    setShoppingCartList: function() {
		var i = 0, itemList = [];
		$(".Odh").each(function() {
            if($(this).hasClass("checkBoxTrue")){
				i++; 
				itemList.push({skuId:$(this).attr("skuId"),qty:$("#qty_"+$(this).attr("itemId")).text()})	
			}
        });
		if(i==0){
			$("#TipDiv").html("至少选择一个商品").show();
			setTimeout(function(){$("#TipDiv").hide()},1500)
			return false; 	
		}
        var jsonarr = {

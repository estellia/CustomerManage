...
namespace JIT.CPOS.Web.RateLetterInterface.PG
{
    ...
    /// GroupHandler 的摘要说明
    /// </summary>
    public class PGGroupHandler : BaseGateway
    {
        ...
        private string CreateGroup(string reqContent)
        {
            ...
            try
            {
                ...
                DataSet dsUser = userBLL.GetPgUserByID(rp.CustomerID, rp.UserID, typeID);
                if (dsUser.Tables != null && dsUser.Tables.Count > 0 && dsUser.Tables[0] != null && dsUser.Tables[0].Rows.Count > 0)
                {
                    //判断band3 >=3的才可以创建聊天组
                    if (Convert.ToInt32(dsUser.Tables[0].Rows[0]["JobFunc"]) < 3)
                    {
    ...
    public class IMGroupContainMembers : IMGroupEntity
    {

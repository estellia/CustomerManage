...
namespace JIT.CPOS.Web.ApplicationInterface.Vip
{
    ...
    public class VipGateway : BaseGateway
    {
        ...
        public string GetVipIntegral(string pRequest)
        {
            ...
            if (rp.Parameters.PayAmount != 0)
            {
                totalPayAmount = rp.Parameters.PayAmount;
            }
            else if (skuIdAndQty != null)
            {
            ...
            //如果订单业务类型为null
            if (rp.Parameters.OrderReasonTypeId == null)
                rp.Parameters.OrderReasonTypeId = "";
            //超级分销、团购、抢购、砍价商品没有会员折扣
            if (!rp.ChannelId.Equals("11") && rp.Parameters.OrderReasonTypeId != "CB43DD7DD1C94853BE98C4396738E00C" && rp.Parameters.OrderReasonTypeId != "671E724C85B847BDA1E96E0E5A62055A" && rp.Parameters.OrderReasonTypeId != "096419BFDF394F7FABFE0DFCA909537F")
            {
        ...
        public string GetVipCoupon(string pRequest)
        {
            ...
            //根据门店编码获取门店信息
            if (!string.IsNullOrEmpty(rp.Parameters.UnitCode))
            {
                var unitBll = new t_unitBLL(loggingSessionInfo);
                var unitEntity = unitBll.QueryByEntity(new t_unitEntity() { unit_code = rp.Parameters.UnitCode }, null).FirstOrDefault();
                if (unitEntity != null)
                {
                    rp.Parameters.ObjectID = unitEntity.unit_id;
                }
            }
            var ds = bll.GetVipCouponDataSet(rp.UserID, totalPayAmount, rp.Parameters.UsableRange, rp.Parameters.ObjectID, rp.Parameters.Type, rp.CustomerID);
        ...
        {
            var rp = pRequest.DeserializeJSONTo<APIRequest<GetVipInfoRP>>();
            var loggingSessionInfo = Default.GetBSLoggingSession(rp.CustomerID, rp.UserID);
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(rp.Parameters.Phone))
                {
                    vipEntity = bll.QueryByEntity(new VipEntity()
                            {
                                Phone = rp.Parameters.Phone,
                                ClientID = rp.CustomerID
                            }, null).FirstOrDefault();
                    //重新获取登陆信息
                    loggingSessionInfo = Default.GetBSLoggingSession(rp.CustomerID, vipEntity.VIPID);
                }
                else if (string.IsNullOrEmpty(rp.UserID) && !string.IsNullOrEmpty(rp.OpenID))
                {
                    vipEntity = bll.QueryByEntity(new VipEntity()
                            {
            ...
            var vipCardVipMappingEntity = vipCardVipMappingBll.QueryByEntity(new VipCardVipMappingEntity() { VIPID = vipEntity.VIPID, CustomerID = loggingSessionInfo.ClientID }, null).FirstOrDefault();
            var vipCardEntity = vipCardBll.GetByID(vipCardVipMappingEntity.VipCardID);
            var VipCardTypeEntity = vipCardTypeBll.GetByID(vipCardEntity.VipCardTypeID);
            //会员卡类型名称
            rd.VipCardTypeName = VipCardTypeEntity.VipCardTypeName;
            ...
            if (vipIntegralEntity != null)
            {
                rd.EndIntegral = vipIntegralEntity.ValidIntegral; //获取有效积分
            }
            else
            {
                rd.EndIntegral = 0;
        ...
        public string SetOrderStatus(string pRequest)
        {
            ...
            using (tran.Connection)
            {
                try
                {
                    ...
                    #region 20161213 增加判断用户余额必须大于总的折扣后金额.
                    if (vipAmountEntity != null)
                    {
                        //20161212 增加判断用户余额必须大于实付金额.
                        Decimal? totalAmount = tInoutEntity.TotalAmount;
                        if (vipDiscount > 0)
                            totalAmount = totalAmount * (vipDiscount / 10);

                        if (vipAmountEntity.EndAmount < totalAmount)
                            throw new APIException(string.Format("账户余额不足，当前余额为【{0}】", vipAmountEntity.EndAmount)) { ErrorCode = 103 };
                    }
                    else
                    {
                        throw new APIException("账户余额为空!") { ErrorCode = 103 };
                    }
                    ...
                    {
                        if (!superRetailTraderItemBll.IsStockEnough(inoutDetailList.ToList()))
                        {
                    ...
                    if (AfterPaySetStock != "1")
                    {
                        if (tInoutEntity.DataFromID == "35" || tInoutEntity.DataFromID == "36")
                        {
                            superRetailTraderItemBll.SetSuperRetailTraderItemStock(inoutDetailList.ToList());
                        }
                    }
                    // 根据订单ID修改订单配送状态
                    //推送ios、android信息
                    ...
                    #region 运费处理
                    CustomerDeliveryStrategyBLL customerDeliveryStrategyBLL = new CustomerDeliveryStrategyBLL(loggingSessionInfo);
                    JIT.CPOS.BS.Entity.CustomerDeliveryStrategyEntity customerDeliveryStrategyEntity = customerDeliveryStrategyBLL.GetDeliveryAmount(loggingSessionInfo.ClientID, (tInoutEntity.TotalAmount ?? 0), tInoutEntity.Field8);

                    decimal deliveryAmount = 0;
                    if (customerDeliveryStrategyEntity.Id != null)
                    {
                        deliveryAmount = customerDeliveryStrategyEntity.DeliveryAmount ?? 0;
                    }
                    #endregion

                    #region 会员折扣
                    decimal tempAmount = (tInoutEntity.TotalAmount ?? 0);
                    if (vipDiscount > 0 && vipDiscountRp > 0)
                    {
                        ...
                        //discountAmount = discountAmount + vipDiscountRp;  //先折后抵，不将会员折扣金额
                        tInoutEntity.DiscountRate = vipDiscount * 10;
                    ...
                    if (integralFlag == 1)
                    {
                        ...
                        tInoutEntity.PayPoints = integral;
                        tInoutEntity.ReceivePoints = integral;

                        var integralPay =
                            paymentTypeList.ToList()
                                .Find(t => t.Payment_Type_Code == "IntegralPay");
                        //入支付明细表
                        var paymentDetail = new T_Payment_detailEntity()
                        {
                            Payment_Id = Guid.NewGuid().ToString(),
                            Inout_Id = tInoutEntity.OrderID,
                            ...
                            Price = integral,
                            Total_Amount = tInoutEntity.TotalAmount,
                            Pay_Points = 0,
                    ...
                    if (couponFlag == 1)
                    {
                        ...
                        var couponPay =
                            paymentTypeList.ToList()
                                .Find(t => t.Payment_Type_Code == "CouponPay");
                        //入支付明细表
                        var paymentDetail = new T_Payment_detailEntity()
                        {
                            Payment_Id = Guid.NewGuid().ToString(),
                    ...
                    #region 现已不使用返现 min.zhang 2016.6.24
                    //使用返现
                    ...
                    if (vipEndAmountFlag == 1)
                    {
                        ...
                        tInoutEntity.Field3 = vipEndAmount.ToString();
                        ...
                        var balancePay =
                            paymentTypeList.ToList()
                                .Find(t => t.Payment_Type_Code == "BalancePay");
                        //入支付明细表
                        var paymentDetail = new T_Payment_detailEntity()
                        {
                            Payment_Id = Guid.NewGuid().ToString(),
                    ...
                    if (tempAmount < discountAmount)
                        ...
                        tInoutEntity.ActualAmount = tempAmount - discountAmount;

                    //如果实付金额 = 各种优惠活动的总和 设置付款状态=1【已付款】 
                    ...
                    if (tInoutEntity.Field1 == "1")
                    {
                        ...
                        try
                        {
                            ...
                            foreach (var tmp in paymentList)
                            {
                                var orderPayment = new EventContract
                                {
                                    Operation = OptEnum.Create,
                                    EntityType = EntityTypeEnum.OrderPayment,
                                    Id = tmp.Payment_Id
                                };
                                eventService.PublishMsg(orderPayment);
                            }
                        }
                        catch (Exception ex)
                        {
    ...
    /// GetVipInfo传入参数
    /// </summary>
    public class GetVipInfoRP : IAPIRequestParameter
    {
        /// <summary>
        /// 会员手机号
        /// </summary>

...
var Vip = {
            ...
            success: function (o) {
                ...
                if (o.code == "200") {
                    ...
                    if (Listdata.length > 0) {
                        ...
                        Vip.getCouponTypeList(orderId, o.content.orderList[0].totalAmount, '0');
    ...
    ///////////////////////////////////////////////////
    getCouponTypeList: function (orderId, totalAmount, loadFlag) {
        //alert(orderId);
        //alert(totalAmount)
        var jsonarr = { 'action': "orderCouponSum", ReqContent: JSON.stringify({ "common": Base.All(), "special": { 'orderId': orderId, 'totalAmount': totalAmount, 'loadFlag': loadFlag} }) };
        $.ajax({
            type: 'get',
            url: url,
            data: jsonarr,
            ...
            success: function (o) {
                if (o.code == "200") {
                    //                    alert(o.content.count);
                    //                    alert(o.content.amount);
                    var ReturnHtml = _.template($('#getCouponList').html(), o.content);
                    $("#divCoupon").html(ReturnHtml);
                    var actualAmount = totalAmount;
                    if (o.content == null || o.content.amount == null || o.content.amount < 0 || o.content.amount > totalAmount) {
                        actualAmount = totalAmount;
                    } else {
                        actualAmount = totalAmount - o.content.amount;
                        //actualAmount = "0.04"; //测试数据
                    }

                    $("#actualAmount").text(actualAmount); //显示的优惠后的金额
                    $("#couponAmount").text(actualAmount); //隐藏的优惠后的金额，用来切换
                    if (o.content.count == 0) {
                        WelfareAction.CDDownCoupon(2);
                    }
                }
            }
        })
    ...
    setUpdateOrderDelivery: function () {
        ...
        var couponsPrompt = $("#LbCouponPrompt").text();
        var actualAmount = $("#couponAmount").text();
        var jsonarr = { 'action': "setUpdateOrderDelivery", ReqContent: JSON.stringify({ "common": Base.All(), "special": { "storeId": getThisStore, "username": $("#vipName").val(), "mobile": mobile, "email": email, "remark": $("#remark").val(), "deliveryId": deliveryId, "deliveryAddress": deliveryAddress, "deliveryTime": deliveryTime, "couponsPrompt": couponsPrompt, "actualAmount": actualAmount, orderId: getParam("orderId")} }) };
        $.ajax({
            type: 'post',
            url: url,
            data: jsonarr,

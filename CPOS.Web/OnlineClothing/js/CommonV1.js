        ...
        $.ajax({
            ...
            success: function (o) {
                ...
                if (o.code == "200") {
                    ...
                    $("#actualAmount").text(o.content.actualAmount.substring(0, totalcc.indexOf(".") + 3));
                    $("#Dea_" + o.content.deliveryid).show();
                    $("#dDea_" + o.content.deliveryid).show();
                    var couponAmount = parseInt(o.content.totalamount) - parseInt(o.content.actualAmount);
...
var WelfareAction = {
    ...
    CDDownCoupon: function (index) {

        if (!$("#CO_" + index).hasClass("HasSelectYY")) {
            $("li[id^=TetCO_]").hide();
            $("#TetCO_" + index).show();
            $("span[id^=CO_]").removeClass("HasSelectYY").addClass("HasSelectNo");
            $("#CO_" + index).removeClass("HasSelectNo").addClass("HasSelectYY");
            if (index == 2) { //&& $.trim($("#AppShopList").text()) == ""
                //Vip.ShopList("1", "vipSureDetail");
                $("#actualAmount").text($("#totalAmount").text());
                $("#couponAmount").text($("#totalAmount").text());
                $("#LbCouponPrompt").text('');
                var jsonarr = { 'action': "cancelCoupon", ReqContent: JSON.stringify({ "common": Base.All(), "special": { "orderId": getParam("orderId"), "couponId": ''} }) };
                $.ajax({
                    type: 'get',
                    url: url,
                    data: jsonarr,
                ...
                $("#actualAmount").text($("#couponAmount").text());
            }
            $("#couponTypeId").val(index);
        }

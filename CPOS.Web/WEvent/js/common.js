...
//var url = '../../publicMark'; 
var url = '/WEvent/weixinData.aspx?method=';
var Timeout = null ;
var GET = new Array(), QueString = '', querystr = new Array(), savebtnScrolltop = 0;
var position_option = {
...
var  Win={
	...
	var Event = { //活动
	    ...
	    ShowApply: function () {
	        var jsonarr = { ReqContent: JSON.stringify({ "common": Base.All(), "special": { "eventId": $("#EventDetailId").attr("eventId")} }) };
	        $.ajax({
	            type: 'get',
	    ...
	    ApplySubmit: function (curobj) {
	        if ($(curobj).hasClass("inputsubmitStylegray")) {
	            return;
	        }
	        $(curobj).addClass("inputsubmitStylegray");
	        var NewsAnswerData = {};
	        ...
	        if (this.GetApplyAnswer("AllQuestionId") != false) {
				...
				$(curobj).removeClass("inputsubmitStylegray");
	            return false;
	        }
	        var userName = "";
	        ...
	        var jsonarr = { Form: JSON.stringify({ "common": Base.All(), "special": { "eventId": $("#EventDetailId").attr("eventId"), questions: NewsAnswerData, userName: userName, mobile: "", email: ""} }) };
	        $.ajax({
	            type: 'post',
	                ...
	                if (o.code == 200) {
	                    ...
	                    if (window.localStorage) {
	                        ...
	                        for (var i = 0; i < oAnswerData.length; i++) {
	                            if (oAnswerData[i].isSaveOutEvent == "1") {
	                                //	Odata[oAnswerData[i].cookieName] = oAnswerData[i].questionValue;
	                                localStorage[oAnswerData[i].cookieName] = oAnswerData[i].questionValue;
	                            } else {
	                                bbc[oAnswerData[i].cookieName] = oAnswerData[i].questionValue;
	                            }
	                        }
	                        //	AloneData[GetEventID] =JSON.stringify(bbc);
	                        localStorage["E_" + GetEventID] = JSON.stringify(bbc);
	                        localStorage["Apply_" + GetEventID] = userName;
	                    }

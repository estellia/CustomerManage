...
var title = '中欧国际工商学院',defaultimgUrl="http://alumniapp.ceibs.edu:8080/ceibs/FileUploadServlet/upfiles/event_default.png";
var appId = 'wx11e23c2e4e930f43',imgUrl =defaultimgUrl, link = document.URL,fakeid = "",desc = desc || link;
weiXinFollow();
function weiXinFollow(){
var pageTitle=$('.newsListp2');//获取页面标题
if (pageTitle.size()) {
	title="简介";
	desc=pageTitle.html();
};
	...
	$("a").live("click",function(){	
		...
		if(getHref.indexOf("customerId")<=-1 && getHref!='javascript:void(0)' && getHref!='javascript:;' && getHref.indexOf("tel:")<=-1&& getHref.indexOf("mailto:")<=-1){
			if(getHref.indexOf("?") > -1){
				location.href = getHref+"&customerId="+getParam("customerId")+"&random="+Math.floor(Math.random()*1000000);
			}else{
				location.href = getHref+"?customerId="+getParam("customerId")+"&random="+Math.floor(Math.random()*1000000);
			}
			return false; 
...
$(function(){
	...
	$("#PcBox").css("min-height",$(window).height()-20)
})
...
var Base = {
	//公共类的获得
		...
		All:function(){
				...
				return {"locale":this.locale(),"userId":this.userId(),"openId":this.openId(), "customerId": this.customerId()};
			}
	}
var  Win={

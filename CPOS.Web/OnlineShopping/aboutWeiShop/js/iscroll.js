...
// INSERT POINT: _init
	...
	_move: function (e) {
		...
		//	newX  = (newX > -(this.currentPage.pageX*320)) ? (this.currentPage.pageX*320) : newX;
			var pagecss = - (this.currentPage.pageX * 320);
			if(newX <= pagecss){
					newX  = pagecss;
				}
		}
				var saveNewX  = pagecss;
		if(this.directionX  == -1){
		//	newX  = (newX > (this.currentPage.pageX*320)) ?(this.currentPage.pageX*320) : newX;
		var pagecsso = -(this.currentPage.pageX * 320);
			//console.log(newX+"."+pagecsso+"."+newY+".."+this.currentPage.pageX+".."+this.directionX)
			if(parseInt(newX) >= parseInt(pagecsso)){
					newX  = pagecsso;
		...
		//console.log(newX+".."+newY+".."+this.currentPage.pageX+".."+this.directionX)
		this.moved = true;
		
		this._translate(newX, newY,this.directionX,this.directionY);

/* REPLACE START: _move */

		if ( timestamp - this.startTime > 398 ) {
...
/* REPLACE END: refresh */
	...
	scrollTo: function (x, y, time, easing) {
		easing = easing || utils.ease.circular;
			 slefc = this;
		if ( !time || (this.options.useTransition && easing.style && slefc.Od ==true)) {
			slefc.Od = false,
			this._transitionTimingFunction(easing.style);
			this._transitionTime(time);
			this._translate(x, y);
			
			setTimeout(function(){slefc.Od = true; },600)
		} else {
...
// INSERT POINT: _transitionTimingFunction
	...
	_translate: function (x, y,dirX,dirY) {
...
/* REPLACE START: _translate */
			...
			/*if(dirX == -1 && this.currentPage.pageX == 2){
				$(".AboutClass3").show();
				$("#BoxBgImg3").hide();	
			}
			if(dirX == -1 && this.currentPage.pageX == 5){
				$(".AboutClass6").show();
				$("#BoxBgImg6").hide();	
			}
			if(dirX == -1 && this.currentPage.pageX == 4){
				$(".AboutClass3").hide();
				$("#BoxBgImg3").show();	
			}
			if(dirX == -1 && this.currentPage.pageX == 7){
				$(".AboutClass6").hide();
				$("#BoxBgImg6").show();	
			}*/
/* REPLACE END: _translate */
	...
	if ( this.indicator1 ) {	// usually the vertical
		this.indicator1.updatePosition(this.currentPage.pageX);
	}

	if ( this.indicator2 ) {
		this.indicator2.updatePosition(this.currentPage.pageX);
	}
...
Indicator.prototype = {
	...
	refresh: function () {
		...
		if ( this.options.listenY ) {
			this.wrapperHeight = this.wrapper.clientHeight;
			if ( this.options.resize ) {
				this.indicatorHeight = Math.max(Math.round(this.wrapperHeight * this.wrapperHeight / this.scroller.scrollerHeight), 10);
	...
	updatePosition: function (Opage) {
		
		//console.log(Opage);
//		if(Opage >= 2 && Opage <=4){
//				//var cgX = this.scroller.x;
//				return;
//			}
//		
		...
		if ( this.scroller.options.useTransform ) {
			...
			this.indicatorStyle.width = (x+NumWindth)+'px';
			this.indicatorStyle.marginLeft =-x+'px'

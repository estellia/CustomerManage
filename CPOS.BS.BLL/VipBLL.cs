...
namespace JIT.CPOS.BS.BLL
{
    ...
    public partial class VipBLL
    {
        ...
        /// 集客销售奖励
        /// </summary>
        public void SetOffActionSalesForOffline(VipEntity vipInfo, T_InoutEntity orderInfo,decimal remainPay)
        {
            decimal actualAmount = remainPay;    //实付金额
            decimal deliveryAmount = 0;      //运费
        ...
        /// 集客销售奖励（收款）
        /// </summary>
        public void SetOffActionSales(VipEntity vipInfo, ReceiveAmountOrderEntity orderInfo)
        {
            decimal actualAmount = orderInfo.AmountAcctPay ?? 0;    //实付金额    //运费
            ...
            if (SetoffEvent.Count > 0)
            {
                foreach (var item in SetoffEvent)
                {
                    ...
                    if (IincentiveRuleData != null)
                    {
                        ...
                        #region 查看已完成订单数量
                        List<IWhereCondition> complexCondition = new List<IWhereCondition> { 
                                                new EqualsCondition() { FieldName = "VipId", Value = vipInfo.VIPID }
                                                ,new EqualsCondition() { FieldName = "PayStatus", Value = "10" }
                                                ,new DirectCondition() { Expression="CreateTime>='"+IincentiveRuleData.CreateTime+"'" }
                                                };

                        var ordercount = new ReceiveAmountOrderBLL(CurrentUserInfo).PagedQuery(complexCondition.ToArray(), null, 1, 1);
                        #endregion
                                ...
                                switch (IincentiveRuleData.SetoffRegAwardType)
                                {
                                    case 1:
                                        ...
                                        if (ResultPer > 0)
                                        {
                                            ...
                                            if (IincentiveRuleData.SetoffOrderTimers == 0)  //单单有效
                                            {
                                                AddVipGoldAmount(ResultMonery, vipInfo.HigherVipID, orderInfo.OrderId.ToString(), "20", "集客行动销售奖励", VipData.VipCode);
                                            }
                                            if (IincentiveRuleData.SetoffOrderTimers == 1) // 首单有效
                                            {
                                                if (ordercount.RowCount == 1)
                                    ...
                                    case 2:
                                        ...
                                        if (ResultPer > 0)
                                        {
                                            ...
                                            {
                                                //单单有效
                                                AddVipGoldIntegral(ResultIntergral, vipInfo.HigherVipID, orderInfo.OrderId.ToString(), "25", "集客行动销售奖励", VipData.VipCode);
                                            }
                                            if (IincentiveRuleData.SetoffOrderTimers == 1)
                                            {
                                                //首单有效
                        ...
                        else if (item.SetoffType == 2)
                        {
                            if (!string.IsNullOrEmpty(vipInfo.SetoffUserId))
                            {
                                ...
                                if (ResultPer > 0)
                                {
                                    ...
                                    {
                                        #region 单单有效
                                        AddVipGoldAmount(ResultMonery, vipInfo.SetoffUserId, orderInfo.OrderId.ToString(), "20", "集客行动销售奖励", "");
                                        #endregion
                                    }
                                    if (IincentiveRuleData.SetoffOrderTimers == 1)
                                    {
        ...
        public void CancelSetOffActionSales(string orderId)
        {
            ...
            #region 取消余额奖励
            //订单奖励余额明细
            var vipAmountDetailEntityList = vipAmountDetailBll.QueryByEntity(new VipAmountDetailEntity() { ObjectId = orderId, AmountSourceId = "20" }, null);
            if (vipAmountDetailEntityList != null)
            {
                foreach (var item in vipAmountDetailEntityList)
                {
                    //余额扣除
                    var vipAmountEntity = vipAmountBll.QueryByEntity(new VipAmountEntity() { VipId = item.VipId }, null).FirstOrDefault();
                    if (vipAmountEntity != null)
                    {
                        vipAmountEntity.ReturnAmount -= item.Amount;//余额
                        vipAmountEntity.ValidReturnAmount -= item.Amount;//累计金额
                        vipAmountEntity.TotalReturnAmount -= item.Amount;//累计金额
                        vipAmountBll.Update(vipAmountEntity);
                        vipAmountDetailBll.Delete(item);
                    }
                }
            }
            #endregion
            ...
            #region 取消积分奖励
            //订单奖励积分明细
            var vipIntegralDetailEntityList = vipIntegralDetailBll.QueryByEntity(new VipIntegralDetailEntity() { ObjectId = orderId, IntegralSourceID = "25" }, null);
            if (vipIntegralDetailEntityList != null)
            {
                foreach (var item in vipIntegralDetailEntityList)
                {
                    //余额扣除
                    var vipIntegralEntity = vipIntegralBll.QueryByEntity(new VipIntegralEntity() { VipID = item.VIPID }, null).FirstOrDefault();
                    if (vipIntegralEntity != null)
                    {
                        vipIntegralEntity.OutIntegral += item.Integral;//余额
                        vipIntegralEntity.EndIntegral -= item.Integral;//累计金额
                        vipIntegralEntity.ValidIntegral -= item.Integral;//累计金额
                        vipIntegralBll.Update(vipIntegralEntity);
                        vipIntegralDetailBll.Delete(item);
                    }
                }
            }
            #endregion
        ...
        /// <returns></returns>
        public DataSet GetVipCouponDataSet(string vipId, decimal totalPayAmount, int usableRange, string objectID, int type,string CustomerId)
        {
            return this._currentDAO.GetVipCouponDataSet(vipId, totalPayAmount, usableRange, objectID, type, CustomerId);
        }

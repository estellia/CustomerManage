...
namespace JIT.CPOS.BS.BLL
{
    ...
    public partial class Agg_UnitBirthdayNoBackVipBLL
    {
        ...
        /// <returns></returns>
        public PagedQueryResult<Agg_UnitBirthdayNoBackVipEntity> PagedQueryForVip(string customerId, string unitId, DateTime dateCode, bool isBack, int pageIndex, int pageSize)
        {
            //查询参数
            ...
            lstWhere.Add(new EqualsCondition() { FieldName = "IsBack", Value = Convert.ToInt32(isBack) });
            lstWhere.Add(new DirectCondition("DateCode='" + new DateTime(dateCode.Year, dateCode.Month, 1).ToString("yyyy-MM-dd") + "' "));

...
namespace JIT.CPOS.BS.BLL
{
    ...
    public class Inout3Service : BaseService
    {
       ...
       string path_unit_id, string timestamp, string InoutSort, bool getDetail = false)//getDetail默认是false，不是取详细信息。
        ...
        /// <param name="unitID"></param>
        /// <returns></returns>
        public IList<InoutInfo> GetOrderListByOrderNos(string orderList)
        {
            try
            {
                var ds = inoutService.GetOrderUnitList(orderList);
                var inoutInfoList = DataTableToObject.ConvertToList<InoutInfo>(ds.Tables[0]);
        ...
        public InoutInfo GetInoutInfoById_lj(string orderId)
        {
            try
            {
                ...
                if (ds != null && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    //积分抵扣
                    if (inoutInfo.IntegralAmountPer != 0)
                    {
                        inoutInfo.pay_pointsAmount = Math.Round(inoutInfo.integral / inoutInfo.IntegralAmountPer,2);
                    }
                    else
                    {

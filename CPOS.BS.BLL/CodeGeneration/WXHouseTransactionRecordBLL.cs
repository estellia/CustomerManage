...
{   
    /// <summary>
    /// 业务处理： 操作历史记录表：购买 支付 赎回（三种操作 都需要记录在此表中）:一个订单购买/赎回/支付 只会有一条记录，以后每次只需更新状态即可 
    /// </summary>
    public partial class WXHouseTransactionRecordBLL
    {
        private LoggingSessionInfo CurrentUserInfo;
        private WXHouseTransactionRecordDAO _currentDAO;
        #region 构造函数
        /// <summary>
        /// 构造函数 
        /// </summary>
        public WXHouseTransactionRecordBLL(LoggingSessionInfo pUserInfo)
        {
            this.CurrentUserInfo = pUserInfo;
            this._currentDAO = new WXHouseTransactionRecordDAO(pUserInfo);
        }
        #endregion
        #region ICRUDable 成员
        /// <summary>

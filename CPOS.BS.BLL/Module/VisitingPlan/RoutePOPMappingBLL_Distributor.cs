...
{   
    ...
    /// 业务处理： 路线、终端(经销商)映射 
    /// </summary>
    public partial class RoutePOPMappingBLL_Distributor :DistributorModuleCRUDBLL
    {
        ...
        public RoutePOPMappingBLL_Distributor(LoggingSessionInfo pUserInfo, string pTableName)
        ...
        #region GetRouteDistributorList
        /// <summary>
        /// 分页数据
        /// </summary>
        ...
        public PageResultEntity GetRouteDistributorList(List<DefindControlEntity> pSearch, int? pPageSize, int? pPageIndex, string routeid)
        {
            ...
            //sql.AppendLine("ROW_NUMBER() OVER( order by case when MappingID is null then 1 else 0 end asc,RPOPM.Sequence asc,main.LastUpdateTime desc) ROW_NUMBER,");
            ...
            //new 这里不需要渠道查询条件
            sql.AppendLine(string.Format(" inner join (select distinct DistributorID from dbo.fnGetDistributorByUserID('{0}',1)) fn_sad on  CONVERT(nvarchar(100),main.DistributorID) = CONVERT(nvarchar(100),fn_sad.DistributorID) ", userEntity.ClientUserID));
            sql.AppendFormat(" left join RoutePOPMapping RPOPM on cast(main.DistributorID as nvarchar(200))=RPOPM.POPID and RPOPM.isdelete=0 and cast(RPOPM.RouteID as nvarchar(200))='{0}' and RPOPM.ClientID={1} ",
                routeid,
                CurrentUserInfo.ClientID);

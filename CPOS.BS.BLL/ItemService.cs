...
namespace JIT.CPOS.BS.BLL
{
    ...
    public class ItemService : BaseService
    {
        ...
        /// <returns></returns>
        public DataSet GetItemProp2List(string itemId, string propDetailId, string prop1DetailId, int type, string eventId)
        {
            return itemService.GetItemProp2List(itemId, propDetailId, prop1DetailId, type, eventId);
        }
        #endregion
        ...
        public bool SetItemInfoNew(ItemInfo itemInfo, out string strError)
        {
            ...
            if (itemInfo.ifservice == 1)
            {
                ...
                //TODO: liangx 2016-09-22 发券代码在支付回调里面，需要修改
                if (itemInfo.VirtualItems != null)//直接在“页面上”添加商品的时候，用于发优惠券的(以后还会有卡)(===重要)
                {
                    itemInfo.VirtualItems.ForEach(t =>
                    {
                        t.SkuId = itemInfo.SkuList.FirstOrDefault().sku_id;
                        t.ItemId = itemInfo.Item_Id;
                        t.Id = Guid.NewGuid();
                        t.CreateBy = this.loggingSessionInfo.UserID;
                        t.CreateTime = DateTime.Now;
                        t.LastUpdateBy = this.loggingSessionInfo.UserID;
                        t.LastUpdateTime = DateTime.Now;
                        t.CustomerId = this.loggingSessionInfo.ClientID;
                        t.IsDelete = 0;
                    });
                    ...
                    var result = bllVirtualItem.BulkInsert(itemInfo.VirtualItems);
                }
                else{   //处理创建会员卡会员卡转商品的（重要）
                 
                    if (entityVirtualItemList != null && entityVirtualItemList.Any())
                    {
                       var entityVirtualItem = entityVirtualItemList.FirstOrDefault();
                    ...
                    {
                        var entityVirtualItem = new T_VirtualItemTypeSettingEntity
                        {
                            Id = Guid.NewGuid(),
                            ItemId = itemInfo.Item_Id,

    ...
    public partial class VipCardVipMappingBLL
    {
        ...
        public void BindVipCard(string vipId, string vipCode, string unitId)
        {
                ...
                if (freeCardTypeInfo != null)
                {
                    ...
                    if (vipCardTypeInfo != null)
                    {
                        ...
                        else//存在更新成现在的
                        {
                            ...
                            wheres.Add(new MoreThanCondition() { FieldName = "status", Value = 0 });
                            wheres.Add(new EqualsCondition() { FieldName = "vipid", Value = vipId });
                            wheres.Add(new EqualsCondition() { FieldName = "clientid", Value = CurrentUserInfo.ClientID });
            ...
            else //更新旧卡 需要确认旧卡是否为卡体系内的卡
            {
                ...
                var oldVipCardInfo = vipCardBLL.QueryByEntity(new VipCardEntity() { VipCardID = vipCardMappingInfo.VipCardID, CustomerID=CurrentUserInfo.ClientID }, null).FirstOrDefault();

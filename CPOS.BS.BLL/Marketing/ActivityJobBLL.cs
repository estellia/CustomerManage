...
namespace JIT.CPOS.BS.BLL.Marketing
{
    ...
    /// </summary>
    public class ActivityJobBLL
    {
        private static LoggingSessionInfo c_loggingSessionInfo;
        private volatile static ActivityJobBLL _instance = null;
        private static readonly object lockHelper = new object();//实例化锁
        private static string customerID = ConfigurationManager.AppSettings["customer_id"];
        private ActivityJobBLL()
        {
        }
        public static ActivityJobBLL CreateInstance()
        {
            if (_instance == null)
            {
                lock (lockHelper)
                {
                    if (_instance == null)
                    {
                        _instance = new ActivityJobBLL();
                        c_loggingSessionInfo = GetBSLoggingSession(customerID, "1");
                        //c_loggingSessionInfo =
                    }
                }
            }
            return _instance;
        ...
        public void MarketingActivityMethod()
        {
            ...
            //{
            //更新商户
            //c_loggingSessionInfo.ClientID = ConfigurationManager.AppSettings["customer_id"];//item.Key;
            //c_loggingSessionInfo.CurrentLoggingManager.Connection_String = ConfigurationManager.AppSettings["Conn"];//item.Value;
            //执行活动送券业务方法
            ActivitySendCoupon();
            //}
        ...
        public void MarketingActivitySendMessigeMethod()
        {
            ...
            //c_loggingSessionInfo.ClientID = ConfigurationManager.AppSettings["customer_id"];//item.Key;
            ////c_loggingSessionInfo.CurrentLoggingManager.Connection_String = ConfigurationManager.AppSettings["Conn"];//item.Value;
            //c_loggingSessionInfo.Conn = ConfigurationManager.AppSettings["Conn"];//item.Value;
            //执行活动发消息业务方法
            ActivitySendMessige();
            //}
        }
        ...
        #region 获取登陆对象
        private static LoggingSessionInfo GetBSLoggingSession(string customerId, string userId)
        {
        ...
        /// <returns></returns>
        private static string GetCustomerConn(string customerId)
        {

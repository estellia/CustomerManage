/****** Object:  StoredProcedure [dbo].[DynamicVipFormSave]    Script Date: 07/24/2014 13:56:52 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DynamicVipFormSave]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[DynamicVipFormSave]
GO
...
-- Description:	Save Dynamic Vip Form Settings
-- =============================================
CREATE PROCEDURE [dbo].[DynamicVipFormSave] --'F9323FFA-80F7-4262-885F-9435D8413E78', 'e703dbedadd943abacf864531decdac1'
	...
	BEGIN TRAN
		...
		BEGIN
			...
			INSERT INTO MobilePageBlock (
			  ...
			  ,[IsDelete]
			  ,[MobileModuleID]) 
			VALUES (@ParentMobilePageBlockID, 'VIP', '页', 1, 1, NULL, @CustomerID, @UserID, GETDATE(), 0, @FormID)
			...
			INSERT INTO MobilePageBlock (
			  ...
			  ,[IsDelete]
			  ,[MobileModuleID]) 
			VALUES ('VIP', '块', 2, 1, @ParentMobilePageBlockID, @CustomerID, @UserID, GETDATE(), 0, @FormID)
		...
		UPDATE mbd SET IsDelete = 1 
			FROM MobileBussinessDefined mbd 
				WHERE mbd.TableName = 'VIP' AND mbd.MobilePageBlockID = @MobilePageBlockID AND mbd.CustomerID = @CustomerID AND IsDelete = 0
		...
		INSERT INTO MobileBussinessDefined (TableName, MobilePageBlockID, ColumnDesc, ColumnDescEn, ColumnName, CorrelationValue, ControlType, MinLength, MaxLength, IsMustDo, EditOrder, CustomerID, CreateBy, CreateTime, IsDelete)
			(
				SELECT 'VIP', @MobilePageBlockID, cbd.ColumnDesc, cbd.ColumnDescEn, cbd.ColumnName, cbd.CorrelationValue, cbd.ControlType, cbd.MinLength, cbd.MaxLength, t.IsMustDo, t.EditOrder, @CustomerID, @UserID, GETDATE(), 0
					FROM @FieldList t

define(['jquery', 'template', 'tools', 'langzh_CN', 'easyui', 'artDialog', 'kkpager', 'kindeditor'], function ($) {
    ...
    var page = {
            ...
            init: function () {
                ...
                this.distribution();
                //获得地址栏参数为Item_Id的值
                var Item_Id = $.util.getUrlParam("Item_Id");
                //vipId保存起来用来做查询交易记录的参数
                this.loadData.addPram.Item_Id = Item_Id;
                // 初始化页面绑定的事件
                this.initEvent();
            ...
            addItem: function () {
                ...
                var that = this;
                var i = 1;
                $(".addItem").click(function (event, currentItemData) {
                    var wd = 200, H = 30;
                    if (that.VirtualItemTypeInfo && that.VirtualItemTypeInfo.length > 0) {
                        ...
                        var str = '<div class="xnType">' +
                            ...
                            '<em class="tit">数量：</em>' +
                            '<label class="selectBox">' +
                            '<input data-text="商品名称" id="mathType" data-flag="Item_Math"  class="easyui-validatebox" data-options="required:true,width:200,height:30" name="mathTypeId" type="text" value="1">' +
                            '<em class="text">张</em>' +
                            '</label>' +
                            '</div>' +
                                ...
                                if (selectData == "VipCard") {
                                    ...
                                    that.loadData.getCouponTypeList(function (couponData) {
                                        ...
                                        if (couponData.Data.CouponTypeList && couponData.Data.CouponTypeList.length > 0) {
                                            ...
                                            //如果是修改数据，加载优惠券内容
                                            if (currentItemData) {
                                                xnTypeDom.find(".commonSelectWrap").eq(1).find(".easyui-combobox").combobox('select', currentItemData.ObjecetTypeId);
                                            }
                        ...
                        //如果是修改数据，加载当前实体
                        if (currentItemData) {
                            xnTypeDom.find(".commonSelectWrap").eq(0).find(".easyui-combobox").combobox('select', currentItemData.VirtualItemTypeId);
                            xnTypeDom.find(".commonSelectWrap").eq(2).find(".easyui-validatebox").val(currentItemData.Item_Count);
                        }
        ...
        initEvent: function () {
            ...
            that.elems.optPanel.delegate("li", "click", function (e) {
                ...
                if (flag != "#nav01") {
                    ...
                    if (issubmit && $('[data-name="ifservice"]').eq(1).hasClass("on")) {
                        $(".xnType").each(function (i, element) {

                            var itemTypeValue = $(element).find(".commonSelectWrap:eq(0) .easyui-combobox").combobox("getValue");
                            var objectIdValue = $(element).find(".commonSelectWrap:eq(1) .easyui-combobox").combobox("getValue");
            ...
            that.elems.section.delegate("#submitBtn", "click", function (e) {
                ...
                if ($(this).data("issubMit")) {
                    ...
                    /*  that.elems.simpleQuery.find(".panelDiv").fadeOut(0).eq(index).fadeIn("slow");
                    if(index==that.elems.optPanel.find("li").length-1){
                    that.elems.submitBtn.html("提交");
                    that.elems.submitBtn.data("issubMit",true);
                    }else{
                    that.elems.submitBtn.html("下一步");
                    ...
                    that.elems.optPanel.find("li").removeClass("on").eq(index).addClass("on");
                    */
                }
                $.util.stopBubble(e);
            });
        ...
        loadDataPage: function () {
            var that = this, wd = 200, H = 30;

            that.loadData.getCommodityInfo(function (dataInfo) {
                    ...
                    if (data.Data) {
                        ...
                        that.loadData.getClassify(function (data) {
                            ...
                            that.loadData.getClassify(function (data) {
                                ...
                                //如果是实物商品，获取配送类型
                                if (dataInfo.data.ifservice == 0) {
                                    for (var i = 0; i < dataInfo.data.ItemDeliverySettings.length; i++) {
                                        $("#bulkBox .checkBox[data-value='" + dataInfo.data.ItemDeliverySettings[i].DeliveryId + "']").addClass('on');
                                    }
                                    //新干线特殊需求，实物商品不可项编辑。
                                    $('#Category').combotree("readonly");
                                    $('[name="Item_Code"]').attr("readonly", "readonly");
                                    $('[name="Item_Name"]').attr("readonly", "readonly");
                                    $("#nav0_1").undelegate(".checkBox", "click");
                                    $("#nav0_1").undelegate(".radio", "click")
                                }
                                    //虚拟商品，获取虚拟商品集合
                                else if (dataInfo.data.ifservice == 1) {
                                    that.loadData.getItemType(function (data) {
                                        if (data.Data.VirtualItemTypeInfo && data.Data.VirtualItemTypeInfo.length > 0) {
                                            data.Data.VirtualItemTypeInfo.push({
                                                "VirtualItemTypeId": -1,
                                            ...
                                            //加载虚拟商品
                                            for (var i = 0; i < dataInfo.data.VirtualItems.length; i++) {
                                                $(".addItem").trigger('click', dataInfo.data.VirtualItems[i]);
                                            }
                                        }else{
                                            $.messager.alert("提示","虚拟商品类型加载失败","error",function(){
                                                $.util.toNewUrlPath("/module/commodity/querylist.aspx");
                                            })
                                        }
                                    });
                                }
                                ...
                                if (dataInfo.data.SalesPromotionList) {  //促销分组赋值
                                    ...
                                    for (var i = 0; i < dataInfo.data.SalesPromotionList.length; i++) {
                                        promotionList.push(dataInfo.data.SalesPromotionList[i].ItemCategoryId);
                                    }
                                    $('#ItemCategoryId').combotree('setValues', promotionList);
                                }
                                //商品图片列表，
                                if (dataInfo.data.ItemImageList && dataInfo.data.ItemImageList.length > 0) {
                                    var imgList = dataInfo.data.ItemImageList;
                                ...
                                //商品详情
                                if (dataInfo.data.ItemPropList && dataInfo.data.ItemPropList.length > 0) {
                                    $.each(dataInfo.data.ItemPropList, function (index, ItemProp) {
                                        $("[data-flag='details']").each(function () {
                                            var me = $(this);

                                            if (me.data("text") == ItemProp.PropertyCodeName) {
                                                if (me.data("type") == "text") {
                                                    me.val(ItemProp.PropertyCodeValue);
                                                } else if (me.data("type") == "htmltextarea") {
                                                    that.elems.editor.insertHtml(ItemProp.PropertyCodeValue);
                                                }
                                            }
                                        });
                                    });
        ...
        //商品销售信息
        nav03Load: function () {
            var that = this, dataInfo = that.elems.dataInfo;

            if (dataInfo && dataInfo.data.T_ItemSkuProp) {
                var proList = []; //有几个规格
                for (var i = 0; i < 6; i++) {
                    var optionId = "prop_{0}_id".format(i), optionName = "prop_{0}_name".format(i);
                    if (dataInfo.data.T_ItemSkuProp[optionId]) {     //判断是否有值
                        proList.push({ prop_id: dataInfo.data.T_ItemSkuProp[optionId], prop_name: dataInfo.data.T_ItemSkuProp[optionName] });
                    } //select   选择事件，
                }
                if (proList.length > 0) { //表示是添加规格保存的sku值， 需要遍历生成skulist表格
                    for (var i = 0; i < proList.length; i++) {
                        ...
                        if (dataInfo.data.SkuList && dataInfo.data.SkuList.length > 0) {   //去子类规格
                            var optionId = "prop_{0}_detail_id".format(i + 1), optionName = "prop_{0}_detail_name".format(i + 1);
                            var proDetailList = []; //缓存当前规格值

                            for (var j = 0; j < dataInfo.data.SkuList.length; j++) {
                                var isAdd = true; //控制重复添加
                                if (proDetailList.length > 0) {   //用于排除以及添加的
                                    for (var k = 0; k < proDetailList.length; k++) {
                                        if (proDetailList[k].prop_detail_id == dataInfo.data.SkuList[j][optionId]) {
                                            isAdd = false;
                                            break;
                                        }
                                    }
                                }
                                if (isAdd && dataInfo.data.SkuList[j][optionId]) {
                                    proDetailList.push({ prop_detail_id: dataInfo.data.SkuList[j][optionId], prop_detail_name: dataInfo.data.SkuList[j][optionName] })
                                }

                            }

                            var btnLitDiv = skuList.eq(i).find(".fontC").siblings(".proValue");
                            for (var k = 0; k < proDetailList.length; k++) {
                                var item = { id: proDetailList[k].prop_detail_id, name: proDetailList[k].prop_detail_name };
                    ...
                    if(dataInfo.data.ifservice==1){
                        ...
                        that.getProData(null,true);
                    }

                    if (dataInfo.data.SkuList && dataInfo.data.SkuList.length > 0) {
                        for (var index = 0; index < dataInfo.data.SkuList.length; index++) {
                            var sku_price_list = dataInfo.data.SkuList[index].sku_price_list;
                            for (var j = 0; j < sku_price_list.length; j++) {
                                $.each(that.elems.allData.ItemPriceTypeList, function (i, Item) {
                        ...
                        that.elems.skuTable.datagrid({ data: dataInfo.data.SkuList });
                        if(dataInfo.data.ifservice==1){
                            for (var index = 0; index < dataInfo.data.SkuList.length; index++) {
                                that.elems.skuTable.datagrid('beginEdit', index);
                            }
                        }else{
                            for (var index = 0; index < dataInfo.data.SkuList.length; index++) {    //为了计算总库存。
                                that.elems.skuTable.datagrid('beginEdit', index);
                                that.elems.skuTable.datagrid('endEdit', index);
                            }

                            //如果不是虚拟商品 规格值不可以编辑，新干线特殊需求。因为商品数据单向同步此处编辑会照成数据不一致问题。
                            $("#batch .commonSelectWrap").eq(0).hide();$("#sku").hide();
                        }
                    ...
                    if (dataInfo.data.SkuList && dataInfo.data.SkuList.length > 0) {
                        var skuInfo = dataInfo.data.SkuList[0];
                        $("#dataState").find("[name='barcode']").val(skuInfo.barcode);
                        for (var i = 0; i < skuInfo.sku_price_list.length; i++) {
                            $("[data-flag='price']").each(function (index, dom) {
                                var me = $(this);
                                if (me.data("text") == skuInfo.sku_price_list[i].item_price_type_name) {
                                    if (me.data("type") == 'price') {

                                        me.numberbox('setValue', skuInfo.sku_price_list[i].sku_price);

                                        if (dataInfo.data.ifservice == 0) {     //如果不是虚拟商品 规格值不可以编辑，新干线特殊需求。因为商品数据单向同步此处编辑会照成数据不一致问题。
                                            me.numberbox("disable", true);
                                        }
                                    }
                                }
                            });
        ...
        //绘制表格
        getProData: function (items,ishide) {
            $("#countRepertory").html("0");
            var logs = 1;
                                ...
                                formatter: function (value, row, rowindex) {
                                    ...
                                    options: {
                                        ...
                                        onChange: function (newValue, oldValue) {
                                            ...
                                            if (number == 0 && oldValue == 0 && newValue == 1) {
                                                //默认值是1 首次加载会出问题。库存被多加1
                                            } else {
            ...
            table.options.columns[0].push(
                {
                    field: "bat_id", title: "操作", width: 40, hidden:ishide,
                    formatter: function (value, row, rowindex) {
                        if (value == 1) {
        ...
        loadData: {
            ...
            getCommodityInfo: function (callback) {
                $.util.oldAjax({
                    url: "/module/basic/Item/Handler/ItemHandler.ashx",
                    data: {
                        action: 'get_item_by_id',
                        item_id: this.addPram.Item_Id     //商品ID

    ...
    cellEditing = Ext.create('Ext.grid.plugin.CellEditing', {
        ...
        listeners: {
            "beforeedit": function (a, b, c) {
                if (checkStore.getById(b.record.data.DistributorID) != null && checkStore.getById(b.record.data.DistributorID).data.IsDelete == 0)
                    return true;
                else
                    return false;
            },
            "edit": function (a, b, c) {
                var r = checkStore.getById(b.record.data.DistributorID);
                var r1 = null;
                for (var i = 0; i < gridStoreList.getStore().data.items.length; i++) {
                    if (b.record.data.DistributorID == gridStoreList.getStore().data.items[i].data.DistributorID) {
    ...
    selModelOrder = Ext.create('Ext.selection.CheckboxModel', {
        ...
        listeners: {
            'deselect': function (a, b, c) {
                var r = checkStore.getById(b.data.DistributorID);
                //checkStore.remove(r);
                r.data.IsDelete = 1;
                //r.data.Sequence = 0;
            },
            'select': function (a, b, c) {
                if (a.checkOnly == true) {
                    if (checkStore.getById(b.data.DistributorID) == null) {
                        checkStore.insert(0, b.data);
                    } else {
                        if (checkStore.getById(b.data.DistributorID).data.IsDelete = 1) {
                            checkStore.getById(b.data.DistributorID).data.IsDelete = 0;
                        }
                    }
                }
            }

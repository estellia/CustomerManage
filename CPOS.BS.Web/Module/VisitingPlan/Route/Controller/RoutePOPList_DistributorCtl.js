...
function fnSearch() {
    ...
    for (var i = 0; i < gridStoreList.getStore().getCount(); i++) {
        ...
        if (gridStoreList.getStore().data.items[i].data.MappingID != "") {
            ...
            if (checkStore.getById(gridStoreList.getStore().data.items[i].data.DistributorID) != null) {
    ...
    for (var i = 0; i < gridStoreList.getStore().getCount(); i++) {
        if (checkStore.getById(gridStoreList.getStore().data.items[i].data.DistributorID) != null
                && checkStore.getById(gridStoreList.getStore().data.items[i].data.DistributorID).data.IsDelete == 0) {

            gridStoreList.getStore().getAt(i).set("Sequence", checkStore.getById(gridStoreList.getStore().data.items[i].data.DistributorID).data.Sequence);
    ...
    if (checkStore.getCount() > 0) {
        for (var i = 0; i < checkStore.getCount(); i++) {
            if (checkStore.data.items[i].data.MappingID == ""
                ...
                b.DistributorID = checkStore.data.items[i].data.DistributorID;

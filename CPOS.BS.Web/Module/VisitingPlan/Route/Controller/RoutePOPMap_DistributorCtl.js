...
function _map_Graphic_OnClick(obj) {
    var storeinfo = fnGetStoreInfoByMapDistributorID(obj.StoreID);
    //alert(storeinfo.DistributorID);
    if (Ext.getStore("storeStore").getById(storeinfo.DistributorID) == null) {
        var r = Ext.create('storeEntity', {
            MappingID: storeinfo.MappingID,
            MapDistributorID: storeinfo.MapDistributorID,
            DistributorID: storeinfo.DistributorID,
            Distributor: storeinfo.Distributor,
...
function fnInitMap() {
    ...
    if (window.frames["frmFlashMap"].index != undefined
        && window.frames["frmFlashMap"].index._map_RemoveStores != undefined
        && gridStoreList != undefined) {
        ...
        for (var i = 0; i < gridStoreList.getStore().getCount(); i++) {
            ...
            //Point.LabelContent = r.data.Distributor;
            Point.Tips = r.data.Distributor;

            PointArray.push(Point);

            //grid数据
            if (r.data.MappingID != "") {
                if (Ext.getStore("storeStore").getById(r.data.DistributorID) == null) {
                    var entity = Ext.create('storeEntity', {
                        MappingID: r.data.MappingID,
                        MapDistributorID: i,
                        DistributorID: r.data.DistributorID,
...
//        }
//        else {
//            window.frames["frmFlashMap"].index._map_MoveToStoreByScale("0", 12);
//        }
    }
}
...
function renderLine() {
    ...
    for (var i = 0; i < Ext.getStore("storeStore").getCount(); i++) {
        ...
        var Point = new Object();
        Point.StoreID = r.data.MapDistributorID;
        Point.Lng = r.data.Coordinate.split(",")[0];
        Point.Lat = r.data.Coordinate.split(",")[1];
        Point.Icon = "g.png";
        Point.IsAssigned = "true";  //默认为true
...
function fnGetStoreInfoByMapDistributorID(mapstoreid) {
    for (var i = 0; i < storeList.length; i++) {
        if (storeList[i].MapDistributorID == mapstoreid) {
            return storeList[i];
        }
    }
}
...
function fnRouteAdjust() {
    try {
        ...
        for (var i = 0; i < Ext.getStore("storeStore").getCount(); i++) {
            ...
            var Point = new Object();
            Point.DistributorID = r.data.DistributorID;
            Point.Distributor = r.data.Distributor;
        ...
        if (result != null && result.length > 0) {
            for (var i = 0; i < result.length; i++) {
                var rID = result[i].DistributorID;
...
function fnSave() {
    ...
    if (Ext.getStore("storeStore").getCount() > 0) {
        ...
        for (var i = 0; i < Ext.getStore("storeStore").getCount(); i++) {
            ...
            b.DistributorID = Ext.getStore("storeStore").getAt(i).data.DistributorID;
            array.push(b);
        }
    }
...
function _map_Extent_Select(obj) {
    if (obj != null) {
        for (var i = 0; i < obj.length; i++) {
            var storeinfo = fnGetStoreInfoByMapDistributorID(obj[i].StoreID);

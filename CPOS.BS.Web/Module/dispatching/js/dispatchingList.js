define(['tools', 'template', 'kkpager', 'artDialog', 'json2', 'ajaxform', 'easyui', 'datetimePicker', 'noviceGuideJS'], function () {
    var page =
        {
            saveDataInfo: {},
            //关联到的类别
            ...
            initEvent: function () {
                    ...
                    that.GetModuleNextStep({
                        ...
                        if (data.NowModule) {
                            var step1 = new noviceGuide("step1", {
                                ...
                                isShow: true,
                                title: "设置配送方式",
                                content: "从送货到家、到店自提中选择至少</br>一种配送方式，让您的云店形成020闭环",
                                ...
                                window.location.href = data.PreModule.Url + "#stepIndex=" + data.PreModule.Url.ModuleStep + "&GuId=" + data.PreModule.ParentModule;
                            }, function () {
                                //closeGuide("Step");
                ...
                //获取时间插件
                $('#dispat_startTime').datetimepicker({
                    datepicker: false,
                    format: 'H:i',
                    step: 5
                });
                $('#dispat_endTime').datetimepicker({
                    datepicker: false,
                    format: 'H:i',
                    step: 5
                });
                ...
                $('.jui-dialog-dispatching .radioBoxer').bind('click', function () {
                    var $this = $(this);
                    $('#jui-dialog-' + that.elems.dataObj.typeid + ' .radioBoxer').removeClass('on');
                    $this.addClass('on');
                    if ($this.hasClass('oneDay')) {
                        //$this.parents('.dispatchingContent').find('.formInputBox').attr('disabled', 'disabled');
                        $this.parents('.dispatchingContent').find('.timeWrap').css({ 'display': 'none' });
                        $this.parents('.dispatchingContent').find('.checkBox').removeClass('on');
                    } else {
                        $this.parents('.dispatchingContent').find('.timeWrap').css({ 'display': 'block' });
                        //$this.parents('.dispatchingContent').find('.formInputBox').attr('disabled', false).parent().css({ 'border': '1px solid #ddd' });
                    }
                });
                ...
                $('.tableWrap').delegate('.operateWrap', 'click', function (e) {
                    ...
                    that.entryInfo(obj, function (data) {
                        ...
                        if (deliveryId == '1') {//送货上门
                            ...
                            $("#dispatching_stockuper").val(item.StockUpPeriod);
                            var quantumList = item.QuantumList || [];
                            if (item.MaxDelivery == "0" || item.MaxDelivery == null) {
                                
                                var splitObj = quantumList[0].Quantum.split('-');
                                $("#dispat_startTime").val(splitObj[0]);
                                $("#dispat_endTime").val(splitObj[1]);
                ...
                $('.jui-dialog').delegate('.saveBtn', 'click', function () {
                    ...
                    if (!radioValue) {
                        ...
                        if (deliveryId == '1') {
                            ...
                            var val = $('.jui-dialog-dispatching .radioBoxer.on').attr('date-type');
                            if ($("#dispatching_stockuper").val() == "") {
                                that.alert('预约时间不能为空！');
                                return;
                            }
                            if (val == "0") {
                                if ($("#dispat_startTime").val() == "" || $("#dispat_endTime").val() == "") {
                                    that.alert('营业时间不能为空！');
                                    return;
                                }
                            }
                            obj.Parameters.QuantumList = [];
                            if (val == 0) {
                                var timeStr = $("#dispat_startTime").val() + '-' + $("#dispat_endTime").val();
                                obj.Parameters.QuantumList.push({ "Quantum": timeStr });
                            }
                            obj.Parameters.MaxDelivery = val;
                            obj.Parameters.StockUpPeriod = $("#dispatching_stockuper").val();

        ...
        public string ImportUnit()
        {
             ...
             if (Request("filePath")!=null && Request("filePath").ToString()!="")
             {
                 try
                 {
					...
					#region 根据Excel地址读取数据
					ExcelHelper m_ExcelHelper = new ExcelHelper(HttpContext.Current.Server.MapPath(strPath));

					DataTable dtResult = m_ExcelHelper.ExcelToDataTable("Sheet1", true);

					//当是集客宝版本时判断导入的门店数量是否超过限制数量
					var bllAPCommon = new APCommonBLL(CurrentUserInfo);
					if (bllAPCommon.GetCustomerVersion(CurrentUserInfo.ClientID) == 3) {

						DataSet dsUnitCount = bllAPCommon.GetCustomerLimtUnitCount(CurrentUserInfo.ClientID);
						if (dsUnitCount.Tables.Count > 0 && dsUnitCount.Tables[0].Rows.Count > 0) {
							int intHadUnitNum = unitService.GetUnitCountByCustomerId(CurrentUserInfo.ClientID);

							if ((dtResult.Rows.Count + intHadUnitNum) >= Convert.ToInt16(dsUnitCount.Tables[0].Rows[0]["units"])) {
								responseData.success = false;
								responseData.status = "100";
								responseData.msg = "导入门店数量超过可使用门店数量上限，请申请购买！";
								return responseData.ToJSON();
							}
						}
					}
					...
					DataSet ds = unitService.ExcelToDb(dtResult, CurrentUserInfo);

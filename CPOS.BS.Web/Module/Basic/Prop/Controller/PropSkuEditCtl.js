Ext.onReady(function () {
    ...
    if (type == "1") {
        ...
        Ext.getCmp("txtInputFlag").store = new Ext.data.Store({
            fields: ["id", "value", "text"],
            data: [
                { "id": "3", "value": "3", "text": "输入文本" },
                { "id": "4", "value": "4", "text": "下拉选项" }
            ]
        });

        //隐藏上级节点
        $("#txtParentId").parent().parent().hide();
        //给类型赋值，隐藏
        $("#txtType").parent().parent().hide();
        Ext.getCmp("txtType").jitSetValue("2");
        
        $("#txtNameTitle").html("<font color='red'>*</font>规格名称");
    }
    else if (type == "2") {
        //隐藏上级节点
        $("#txtParentId").parent().parent().hide();
        //给类型赋值，隐藏
        $("#txtType").parent().parent().hide();
        Ext.getCmp("txtType").jitSetValue("3");
        //输入类型，赋值&隐藏
        $("#txtInputFlag").parent().parent().hide();
        Ext.getCmp("txtInputFlag").jitSetValue("2");
        $("#txtNameTitle").html("<font color='red'>*</font>规格值");
    }  
...
function fnSave() {
    ...
    prop.Prop_Code = Ext.getCmp("txtName").jitGetValue();

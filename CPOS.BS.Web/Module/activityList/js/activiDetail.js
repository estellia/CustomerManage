define(['jquery','template','tools','langzh_CN','easyui','artDialog','kkpager','kindeditor'],function($){
    ...
    var page = {
		...
		getFirstStep:function(){
			//
			var that = this;
			$.util.getVersion(function(ver){
				that.activityWay('081AEC92-CC16-4041-9496-B4F6BC3B11FC',ver);//081AEC92-CC16-4041-9496-B4F6BC3B11FC
			});
	        $.util.ajax({
				url: that.elems.domain+"/ApplicationInterface/Module/WEvents/EventsSaveHandler.ashx",
				data: {
			...
			$.util.ajax({
				...
				success: function (data) {
					if (data.IsSuccess && data.ResultCode == 0) {
						...
						if(ver==3){
							LEventDrawMethodList = [{ DrawMethodID: '4', DrawMethodName: '红包' }];
						}else{
							LEventDrawMethodList = [{ DrawMethodID: '2', DrawMethodName: '大转盘' }, { DrawMethodID: '4', DrawMethodName: '红包' }, { DrawMethodID: '5', DrawMethodName: '花样问卷' }];
						}

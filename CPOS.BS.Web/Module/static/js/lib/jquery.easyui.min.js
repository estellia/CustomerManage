...
_26f.mask=$("<div class=\"window-mask\"></div>").insertAfter(_26f.window);
  var height= opts.inline?_26f.mask.parent().height():_273().height;
    if(opts.top+opts.height>height){
        height=opts.top+opts.height;
    }
_26f.mask.css({width:(opts.inline?_26f.mask.parent().width():_273().width),height:height,display:"none"});
}
if(_26f.shadow){
_26f.shadow.remove();
}
...
$(window).resize(function(){
    if($("body>div.window-mask").height()<  $(window)._outerHeight()) {
        $("body>div.window-mask").css({width: $(window)._outerWidth(), height: $(window)._outerHeight()});
    }  else{
        $("body>div.window-mask").css({width: $(window)._outerWidth()});
    }
setTimeout(function(){
    if($("body>div.window-mask").height()<  _273().height) {
...
if(ed.actions.resize){
    ...
    //自定义easyui 编辑框的宽度
    if(col.editor.options&&col.editor.options.width){
        ed.actions.resize(ed.target,col.editor.options.width);
    }

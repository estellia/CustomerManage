...
fnSearch = function (vipSourceId) {
    ...
    var store = Ext.getStore("UnitRewardStore");
    store.proxy.url = JITPage.HandlerUrl.getValue()
        + "&method=search_UnitReward";
...
function fnRenderVipColumn(a, b, c) {
    if (a > 0) {
        return "<a style='color:blue'  target='_blank' href='/module/Vip/VipSearchNew/Vip.aspx?mid=" + __mid + "&type=3&paramValue=" + c.data.UnitId + "'>" + a + "</a>";
    }
    return a;
}
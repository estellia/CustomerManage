...
fnSearch = function (vipSourceId) {
    ...
    var store = Ext.getStore("VipRewardStore");
    store.proxy.url = JITPage.HandlerUrl.getValue()
        + "&method=search_VipReward";
...
function fnRenderVipColumn(a, b, c) {
    if (a > 0) {
        return "<a style='color:blue' target='_blank' href='/module/Vip/VipSearchNew/Vip.aspx?mid=" + __mid + "&type=1&paramValue=" + c.data.VIPID + "'>" + a + "</a>";
    }
    return a;
}

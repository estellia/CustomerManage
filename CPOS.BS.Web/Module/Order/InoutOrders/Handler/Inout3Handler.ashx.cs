...
namespace JIT.CPOS.BS.Web.Module.Order.InoutOrders.Handler
{
    ...
    public class Inout3Handler : JIT.CPOS.BS.Web.PageBase.JITCPOSAjaxHandler
    {
        ...
        protected override void AjaxRequest(HttpContext pContext)
        {
            ...
            switch (pContext.Request.QueryString["method"])
            {
                ...
                //Bear Begin
                case "GetChangeUnitLogList":
                    content = GetChangeUnitLogList();
                    break;
                //Bear End
        ...
        public string GetDeliveryData()
        {
            ...
            var propNameList = skuService.GetSkuInfoByOne(orderId);//"商品规格"属性名称
            var propValueList =
                inoutService.GetInoutDetailInfoByOrderId(orderId, this.CurrentUserInfo.ClientID).OrderBy(t => t.item_code).ToList(); //"商品规格"属性值
            ...
            propDetailName = new StringBuilder(propDetailName.ToString().TrimEnd(';'));
            ...
            var r = new InoutDeliveryData(); //打印配送单所需数据
            ...
            DataTable dtCopy = detail.Tables[0].Copy();
            DataView dv = detail.Tables[0].DefaultView;
            dv.Sort = "ItemCode";
            dtCopy = dv.ToTable();
            ...
            if (returnAmountEntity != null)
            {
                ...
                r.ReturnAmount = 0;
            r.actualAmount= orderInfo.actual_amount + r.vipEndAmount;
            r.vipEndAmount = Math.Abs(r.vipEndAmount.Value);
            ...
            if (orderInfo.discount_rate > 0)
            {
                ...
                var tempAmount = orderInfo.total_amount - r.E9; //应付-运费后的应付金额
                //Bear 注释
                //r.VipDiscountAmount = tempAmount / (orderInfo.discount_rate / 100) - tempAmount;// (应付-运费)/折扣率=去除折扣后实付Y；Y-包含折扣的实付=会员折扣
                r.VipDiscountAmount = tempAmount - tempAmount * (orderInfo.discount_rate / 100);
            }
        ...
        private void Export_lj(HttpContext pContext)
        {
            ...
            //    sheetOneCells.SetColumnWidth(2, 90);
                ...
                //订单总金额，实付金额，会员折扣（换算成金额，会员折扣减去的费用，如100元订单，会员折扣为9折，则此处显示10元）
                //，余额抵扣，优惠券抵扣，积分抵扣（换算成金额），配送费用。
                sheetOneCells[1, 6].PutValue("订单总金额");
                sheetOneCells[1, 6].SetStyle(style2);
                sheetOneCells[1, 7].PutValue("实付金额");
                sheetOneCells[1, 7].SetStyle(style2);
                sheetOneCells[1,8].PutValue("会员折扣");
                sheetOneCells[1,8].SetStyle(style2);
                sheetOneCells[1, 9].PutValue("余额抵扣");
                sheetOneCells[1, 9].SetStyle(style2);
                sheetOneCells[1, 10].PutValue("优惠券折扣");
                sheetOneCells[1, 10].SetStyle(style2);
                sheetOneCells[1, 11].PutValue("积分抵扣");
                sheetOneCells[1, 11].SetStyle(style2);
                sheetOneCells[1, 12].PutValue("配送费用");
                sheetOneCells[1, 12].SetStyle(style2);

                sheetOneCells[1, 13].PutValue("发票抬头");
                sheetOneCells[1, 13].SetStyle(style2);

                sheetOneCells[1,14].PutValue("订单备注");
                sheetOneCells[1, 14].SetStyle(style2);

                sheetOneCells[1, 15].PutValue("消费门店");
                sheetOneCells[1, 15].SetStyle(style2);

                sheetOneCells[1, 16].PutValue("会员姓名");
                sheetOneCells[1, 16].SetStyle(style2);

                sheetOneCells[1, 17].PutValue("支付状态");
                sheetOneCells[1, 17].SetStyle(style2);

                sheetOneCells[1, 18].PutValue("支付方式");
                sheetOneCells[1, 18].SetStyle(style2);

                sheetOneCells[1, 19].PutValue("支付时间");
                sheetOneCells[1, 19].SetStyle(style2);

                sheetOneCells[1, 20].PutValue("配送方式");
                sheetOneCells[1, 20].SetStyle(style2);

                sheetOneCells[1, 21].PutValue("提货人/收货人");
                sheetOneCells[1, 21].SetStyle(style2);

                sheetOneCells[1, 22].PutValue("电话");
                sheetOneCells[1, 22].SetStyle(style2);

                sheetOneCells[1, 23].PutValue("收货地址");
                sheetOneCells[1, 23].SetStyle(style2);

                sheetOneCells[1, 24].PutValue("订单来源");
                sheetOneCells[1, 24].SetStyle(style2);
                ...
                for (int i = 0; i < data.InoutInfoList.Count; i++)
                {
                    ...
                    sheetOneCells[2 + i, 7].PutValue(data.InoutInfoList[i].actual_amount);
                    sheetOneCells[2 + i, 7].SetStyle(style3);
                    sheetOneCells[2 + i, 8].PutValue(data.InoutInfoList[i].discountAmount);
                    sheetOneCells[2 + i, 8].SetStyle(style3);
                    sheetOneCells[2 + i, 9].PutValue(data.InoutInfoList[i].remainSumDis);
                    sheetOneCells[2 + i, 9].SetStyle(style3);
                    sheetOneCells[2 + i,10].PutValue(data.InoutInfoList[i].couponParValue);
                    sheetOneCells[2 + i, 10].SetStyle(style3);
                    sheetOneCells[2 + i, 11].PutValue(data.InoutInfoList[i].PayPointsDis);
                    sheetOneCells[2 + i, 11].SetStyle(style3);
                    sheetOneCells[2 + i, 12].PutValue(data.InoutInfoList[i].DeliveryAmount);
                    sheetOneCells[2 + i, 12].SetStyle(style3);
                
                


                    sheetOneCells[2 + i, 13].PutValue(data.InoutInfoList[i].Field19);
                    sheetOneCells[2 + i, 13].SetStyle(style3);

                    sheetOneCells[2 + i, 14].PutValue(data.InoutInfoList[i].remark);
                    sheetOneCells[2 + i, 14].SetStyle(style3);

                    sheetOneCells[2 + i, 15].PutValue(data.InoutInfoList[i].sales_unit_name);
                    sheetOneCells[2 + i, 15].SetStyle(style3);

                    sheetOneCells[2 + i, 16].PutValue(data.InoutInfoList[i].vip_name);
                    sheetOneCells[2 + i, 16].SetStyle(style3);

                    sheetOneCells[2 + i, 17].PutValue(data.InoutInfoList[i].Field1 == "1" ? "已付款" : "未付款");
                    sheetOneCells[2 + i, 17].SetStyle(style3);

                    sheetOneCells[2 + i, 18].PutValue(data.InoutInfoList[i].payment_name);
                    sheetOneCells[2 + i, 18].SetStyle(style3);

                    sheetOneCells[2 + i, 19].PutValue(data.InoutInfoList[i].complete_date);
                    sheetOneCells[2 + i, 19].SetStyle(style3);

                    sheetOneCells[2 + i, 20].PutValue(data.InoutInfoList[i].DeliveryName);
                    sheetOneCells[2 + i, 20].SetStyle(style3);

                    sheetOneCells[2 + i, 21].PutValue(data.InoutInfoList[i].linkMan);
                    sheetOneCells[2 + i, 21].SetStyle(style3);

                    sheetOneCells[2 + i, 22].PutValue(data.InoutInfoList[i].linkTel);
                    sheetOneCells[2 + i, 22].SetStyle(style3);

                    sheetOneCells[2 + i, 23].PutValue(data.InoutInfoList[i].address);
                    sheetOneCells[2 + i, 23].SetStyle(style3);

                    sheetOneCells[2 + i, 24].PutValue(data.InoutInfoList[i].data_from_name);
                    sheetOneCells[2 + i, 24].SetStyle(style3);
        ...
        public string setPropDetailName(InoutDetailInfo pPropDetailName)
        {
            ...
            if (propDetailName != string.Empty && !string.IsNullOrEmpty(pPropDetailName.prop_4_detail_name) && !string.IsNullOrEmpty(pPropDetailName.prop_4_name))
                propDetailName = propDetailName + "," + pPropDetailName.prop_4_name.ToString() + ":" + pPropDetailName.prop_4_detail_name.ToString();
            else if (propDetailName == string.Empty && string.IsNullOrEmpty(pPropDetailName.prop_4_detail_name) && !string.IsNullOrEmpty(pPropDetailName.prop_4_name))
                propDetailName = propDetailName + pPropDetailName.prop_4_name.ToString() + ":" + pPropDetailName.prop_4_detail_name.ToString();

            if (propDetailName != string.Empty && !string.IsNullOrEmpty(pPropDetailName.prop_5_detail_name) && !string.IsNullOrEmpty(pPropDetailName.prop_5_name))
                propDetailName = propDetailName + "," + pPropDetailName.prop_5_name.ToString() + ":" + pPropDetailName.prop_5_detail_name.ToString();
            else if (propDetailName == string.Empty && string.IsNullOrEmpty(pPropDetailName.prop_5_detail_name) && !string.IsNullOrEmpty(pPropDetailName.prop_5_name))
                propDetailName = propDetailName + pPropDetailName.prop_5_name.ToString() + ":" + pPropDetailName.prop_5_detail_name.ToString();
        ...
        private string SaveDeliveryInfo()
        {
            ...
            if (!string.IsNullOrWhiteSpace(parms["Carrier_id"]))
            {
                if (parms["DeliveryType"] == "1")
            ...
            if (!string.IsNullOrWhiteSpace(parms["UnitId"]))
            {//门店ID
                dict.Add("unit_id", parms["UnitId"]);
                dict.Add("sales_unit_id", parms["UnitId"]);
                dict.Add("purchase_unit_id", parms["UnitId"]);
                if (parms["DeliveryType"] == "2")
                    dict.Add("Carrier_id", parms["UnitId"]);
            }
                ...
                if (order != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(status))
                    {
                        ...
                        if (info.OrderStatus == 10000)
                        {
                            ...
                            T_InoutEntity inoutEntity = inoutBLL.GetByID(order.order_id); //获取订单
                            //会员
                            var vipBll = new VipBLL(CurrentUserInfo);
                            var vipInfo = vipBll.GetByID(inoutEntity.vip_no);//获取会员信息
                            ...
                            if (order.data_from_id == "35" || order.data_from_id == "36")
                            {
                                ...
                                bllSuperRetailTraderOrder.SetRedisToSuperRetailTraderOrder(CurrentUserInfo, inoutEntity);
                            }
                            //购卡
                            if (!string.IsNullOrEmpty(inoutEntity.Field17) && !string.IsNullOrEmpty(inoutEntity.Field18))
                            {
                                //会员卡升级
                                vipCardVipMappingBll.BindVirtualItem(vipInfo.VIPID, vipInfo.VipCode, inoutEntity.sales_unit_id, Convert.ToInt32(inoutEntity.Field18), orderId: inoutEntity.order_id);

                                //分润计算
                                RedisSalesVipCardOrderBLL redisSalesVipCardOrderBll = new RedisSalesVipCardOrderBLL();
                                redisSalesVipCardOrderBll.SetRedisSalesVipCardOrder(CurrentUserInfo, inoutEntity);
                                //售卡处理积分、返现、佣金[完成订单]
                                new SendOrderRewardMsgBLL().OrderReward(inoutEntity, CurrentUserInfo, null);//存入到缓存
                            }
                            else
                            {
                                RedisCalculateVipConsumeForUpgrade redisCalculateVipConsumeForUpgrade = new RedisCalculateVipConsumeForUpgrade();
                                redisCalculateVipConsumeForUpgrade.SetVipConsumeForUpgradeList(CurrentUserInfo, inoutEntity);
                            }

                            //  add by liangx 2016-09-27 获取订单中的虚拟商品 start 
                            CouponTypeBLL couponTypeBLL = new CouponTypeBLL(CurrentUserInfo);
                            RedisVipMappingCouponBLL redisVipMappingCouponBLL = new RedisVipMappingCouponBLL();

                            var couponTypes = couponTypeBLL.GetVirtualItemCouponTypes(inoutEntity.order_id);

                            BaseService.WriteLog("准备向 Redis 发送优惠券, 数量为" + (couponTypes.Any() ? couponTypes.Sum(t => t.Item_Count) : 0 ));
                            ...
                            {
                                //普通商品处理库存和销量
                                if (order.order_reason_id == "2F6891A2194A4BBAB6F17B4C99A6C6F5" && order.data_from_id != "35" && order.data_from_id != "36") //普通商品订单判断
                                {
                        ...
                        try
                        {
                            var newOrder = inoutService.GetInoutInfoById(orderId);

                            if (status == "10000" && newOrder.Field1 == "1")
                            {
        ...
        {
            var order = new Inout3Service(CurrentUserInfo).GetInoutInfoById(Request("order_id").ToString());
            IList<TInOutStatusNodeEntity> list = null;
            if (order != null)
            {
                string order_id = Request("order_id").ToString();
                list = new TInOutStatusNodeBLL(CurrentUserInfo).GetOrderStatusList(Request("order_id"), null, order.Field8);
            }
        ...
        public string GetPosOrder3TotalCount()
        {
            ...
            #region Bear 把“已发货/已服务/已提货”3个状态合并成“已处理”显示 自定义650，Get时对应在拆分 Inout3Service.cs Line 1150
            //获取已处理3个状态的订单数量
            //620, '已服务'
            //610, '已提货'
            //600, '已发货'
            var newStatusCount =
                data.StatusManagerList.ToList()
                    .FindAll(d => d.StatusType == 600 || d.StatusType == 610 || d.StatusType == 620)
                    .Sum(d => d.StatusCount);

            var addIndex = data.StatusManagerList.ToList().FindIndex(t => t.StatusType == 600);

            data.StatusManagerList =
                data.StatusManagerList.Where(d => d.StatusType != 600 && d.StatusType != 610 && d.StatusType != 620)
                    .ToList();

            var newStatus = new StatusManager()
            {
                StatusType = 650,
                StatusTypeName = "已处理",
            ...
            data.StatusManagerList.Insert(addIndex,newStatus);
            #endregion

            responseData.data = data.StatusManagerList;
        ...
        public string SetUnit()
        {
            ...
            var inoutService = new Inout3Service(CurrentUserInfo);
            var vipBLL = new VipBLL(CurrentUserInfo);
            var unitBll = new UnitBLL(CurrentUserInfo);
            var inoutStatus = new TInoutStatusBLL(CurrentUserInfo);
            ...
            if (result > 0)
            {
                ...
                var tmpList = inoutService.GetOrderListByOrderNos(orderList);
                var inoutStatusList = new List<TInoutStatusEntity>();

                foreach (var entity in tmpList)
                {
                    var oldUnit = unitBll.GetUnitDetail(entity.sales_unit_id);
                    TInoutStatusEntity info = new TInoutStatusEntity();
                    info.InoutStatusID = Guid.Parse(Utils.NewGuid());
                    info.OrderID = entity.order_id;
                    info.CustomerID = CurrentUserInfo.CurrentLoggingManager.Customer_Id;
                    info.OrderStatus = Convert.ToInt32(entity.Field7);
                    info.StatusRemark = "订单门店变更[指派给" + oldUnit.unit_name + "]";
                    info.CheckResult = 1;
                    info.CreateTime=DateTime.Now;
                    info.CreateBy = CurrentUserInfo.UserID;
                    info.LastUpdateBy = CurrentUserInfo.UserID;
        ...
        private string GetChangeUnitLogList()
        {
            ...
            int pageSize = Request("limit").ToInt();
            int pageIndex = Request("page").ToInt();

            int rowCount = 0;

            var list = new TInoutStatusBLL(CurrentUserInfo).GetListForChangeUnit(entity, pageIndex, pageSize, out rowCount);

            UnitChangeModel changeModel = null;
            var changeList = new List<UnitChangeModel>();
            foreach (var tmp in list)
            {
                changeModel = new UnitChangeModel();
                 string str= tmp.StatusRemark.Replace("订单门店变更[", string.Empty);
                str = str.TrimEnd(']');
                changeModel.Log = str;
                changeModel.Time = tmp.CreateTime.Value.ToString("yyyy-MM-dd HH:mm:ss");
                changeList.Add(changeModel);
            }

            //return string.Format("{{\"totalCount\":{1},\"list\":{0}}}",  .ToJSON(), rowCount);
    ...
    public class InoutDeliveryData
    {
        ...
        // 商品规格
        // </summary>
        public string PropDetailName { get; set; }

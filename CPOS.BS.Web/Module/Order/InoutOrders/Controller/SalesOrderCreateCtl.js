...
function fnAddItem(id, op, param) {
    ...
    var win = Ext.create('jit.biz.Window', {
        ...
        title: "商品",
        url: "SalesOutOrderItemCreate.aspx?op=" + op + "&id=" + id + getStr(param)
    });
    win.show();
}
...
function fnSave() {
    ...
    var vipNo = Ext.getCmp("vipNo").getValue();
    ...
    if (order.vip_no == null || order.vip_no == "") {
        showError("必须选择用户");
        return;
    }
    ...
    Ext.Ajax.request({
        ...
        sync: true,
        async: false,
        url: 'Handler/InoutHandler.ashx?method=SalesOrder_Create&order_id=' + order_id,

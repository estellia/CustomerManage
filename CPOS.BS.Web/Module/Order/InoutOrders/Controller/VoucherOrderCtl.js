...
function fnCreate() {
    var win = Ext.create('jit.biz.Window', {
        ...
        id: "VoucherOrderEdit",
        title: "结算凭证",
        url: "VoucherOrderEdit.aspx?mid=" + __mid + "&op=new"
    });
    win.show();
...
function fnSearch() {
    Ext.getStore("voucherOrderStore").proxy.url = JITPage.HandlerUrl.getValue()
        + "&method=VoucherOrder";
    Ext.getStore("voucherOrderStore").pageSize = JITPage.PageSize.getValue();
    Ext.getStore("voucherOrderStore").proxy.extraParams = {
        form: Ext.JSON.encode(Ext.getCmp("searchPanel").getValues())
        //, purchase_unit_id: Ext.getCmp("txtPurchaseUnit").jitGetValue()
        , sales_unit_id: Ext.getCmp("txtSalesUnit").jitGetValue()
    };
    Ext.getStore("voucherOrderStore").load();
}
...
function fnView(id, op) {
    ...
    var win = Ext.create('jit.biz.Window', {
        ...
        id: "VoucherOrderEdit",
        title: "结算凭证",
        url: "VoucherOrderEdit.aspx?order_id=" + id + "&op=" + op
    });
    win.show();

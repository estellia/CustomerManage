...
function fnGetOrderNo(op) {
    if (op == "new") {
        Ext.getCmp("txtOrderNo").setValue(newOrderNo("new_order_no_rt"));
    }
}
...
function fnClose() {
    CloseWin('RetailReturnOrderEdit');
}
...
function fnSave() {
    ...
    Ext.Ajax.request({
        ...
        sync: true,
        async: false,
        url: 'Handler/InoutHandler.ashx?method=RetailReturnOrder_Save&order_id=' + order_id,

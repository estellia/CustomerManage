...
function fnGetOrderNo(op) {
    if (op == "new") {
        Ext.getCmp("txtOrderNo").setValue(newOrderNo("new_order_no_aj"));
    }
}
...
function fnAddItem(id, op, param) {
    ...
    var win = Ext.create('jit.biz.Window', {
        ...
        id: "AjOrderItem",
        title: "调整商品",
        url: "AjOrderItem.aspx?op=1&id=" + id + getStr(param)
    });
    win.show();
}
...
//    order.total_amount = parseFloat(tbTotalAmountCtrl.getValue());
    ...
    Ext.Ajax.request({
        ...
        sync: true,
        async: false,
        url: 'Handler/InoutHandler.ashx?method=AjOrder_Save&order_id=' + order_id,

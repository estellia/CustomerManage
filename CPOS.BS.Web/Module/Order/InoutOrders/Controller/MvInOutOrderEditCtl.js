    ...
    if (order_id != "null" && order_id != "") {
        Ext.Ajax.request({
            ...
            success: function (response) {
                ...
                Ext.getCmp("txtOrderNo").setValue(getStr(d.order_no));      //订单号   
                Ext.getCmp("txtOrderDate").setValue(getStr(d.order_date));  //单据日期
                Ext.getCmp("txtOrderType").setValue(getStr(d.order_reason_id)); //单据类型
                Ext.getCmp("txtSalesUnitId").jitSetValue([{ "id": d.sales_unit_id, "text": d.sales_unit_name}]); //出库单位
                Ext.getCmp("txtPurchaseUnitId").jitSetValue([{ "id": d.purchase_unit_id, "text": d.purchase_unit_name}]);//入库单位

                Ext.getCmp("txtPurchaseWarehuouse").fnLoad();   //入库仓库
                Ext.getCmp("txtPurchaseWarehuouse").jitSetValue(d.purchase_warehouse_id);
                Ext.getCmp("txtSalesWarehuouse").fnLoad();   //出库仓库
                Ext.getCmp("txtSalesWarehuouse").jitSetValue(d.sales_warehouse_id);
...
function fnGetOrderNo(op) {
    if (op == "new") {
        Ext.getCmp("txtOrderNo").setValue(newOrderNo("new_order_no_mv_inout"));
    }
}

function fnLoadItems() {
    var storeId = "salesOutOrderEditItemStore"; //purchaseInOrderEditItemStore
    //debugger;
...
function fnAddItem(id, op, param) {
    ...
    var win = Ext.create('jit.biz.Window', {
        ...
        id: "MvInOutOrderItem",
        title: "调拨商品选择",
        url: "MvInOutOrderItem.aspx?op=" + op + "&id=" + id + getStr(param)
    });
    win.show();
}
...
function fnSave() {
    ...
    var hdOrderTypeCtrl = Ext.getCmp("txtOrderType"); //
    var hdSalesUnitIdCtrl = Ext.getCmp("txtSalesUnitId");
    var hdPurchaseUnitIdCtrl = Ext.getCmp("txtPurchaseUnitId");
    var hdPurchaseWarehouseCtrl = Ext.getCmp("txtPurchaseWarehuouse");
    var hdSalesWarehouseCtrl = Ext.getCmp("txtSalesWarehuouse");
    ...
    var order_reason_id = hdOrderTypeCtrl.jitGetValue(); //单据类型
    var order_id = getUrlParam("order_id"); //订单标识
    var order_no = tbOrderCodeCtrl.getValue();//订单号
    var order_date = Ext.getCmp("txtOrderDate").jitGetValueText();//单据日期

    var sales_unit_id = hdSalesUnitIdCtrl.jitGetValue();    //出库单位
    var purchase_unit_id = hdPurchaseUnitIdCtrl.jitGetValue();// 入库单位
    var sales_warehouse_id = hdSalesWarehouseCtrl.jitGetValue(); // 出库单位
    var purchase_warehouse_id = hdPurchaseWarehouseCtrl.value;
    ...
    order.sales_warehouse_id = sales_warehouse_id;
    order.purchase_unit_id = purchase_unit_id;
    order.purchase_warehouse_id = purchase_warehouse_id;
    order.order_reason_id = order_reason_id;
    order.remark = remark;
    ...
    if (order.order_reason_id == null || order.order_reason_id == "") {
        showError("必须选择单据类型");
        return;
    }
    ...
    if (order.sales_warehouse_id == null || order.sales_warehouse_id == "") {
        showError("必须选择出库仓库");
        return;
    }
    ...
    if (order.purchase_warehouse_id == null || order.purchase_warehouse_id == "") {
        showError("必须选择入库仓库");
        return;
    }
    ...
    Ext.Ajax.request({
        ...
        sync: true,
        async: false,
        url: 'Handler/InoutHandler.ashx?method=MvInOutOrder_Save&order_id=' + order_id,

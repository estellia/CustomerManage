    ...
    if (order_id != "null" && order_id != "") {
        Ext.Ajax.request({
            ...
            success: function (response) {
                var storeId = "purchaseInOrderEditStore";
                ...
                Ext.getCmp("txtPurchaseWarehuouse").fnLoad();
                Ext.getCmp("txtPurchaseWarehuouse").jitSetValue(d.warehouse_id);
                //Ext.getCmp("txtPurchaseWarehuouse").jitSetValue([{ "id": d.purchase_unit_id, "text": d.purchase_unit_name }]);
...
function fnGetOrderNo(op) {
    if (op == "new") {
        Ext.getCmp("txtOrderNo").setValue(newOrderNo("new_order_no_ro"));
    }
}
...
function fnAddItem(id, op, param) {
    ...
    var win = Ext.create('jit.biz.Window', {
        jitSize: "big",
        height: 380,
        id: "PurchaseInOrderItem",
        title: "商品",
        url: "PurchaseInOrderItem.aspx?op=" + op + "&id=" + id + getStr(param)
    });
    win.show();
}
...
function fnSave() {
//    //debugger;
    var _grid = Ext.getStore("purchaseInOrderEditItemStore");
    var order = {};
    var flag = false;

    var hdSupplierUnitIdCtrl = Ext.getCmp("txtSalesUnitId");
    var hdPurchaseUnitIdCtrl = Ext.getCmp("txtPurchaseUnitId");
    var hdWarehouseCtrl = Ext.getCmp("txtPurchaseWarehuouse");
...
//    alert(purchase_unit_id)
    ...
    Ext.Ajax.request({
        ...
        sync: true,
        async: false,
        url: 'Handler/InoutHandler.ashx?method=PurchaseInOrder_Save&order_id=' + order_id,

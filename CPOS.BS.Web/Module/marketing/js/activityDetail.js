    ...
    var page = {
        ...
        initEvent: function () {
                        ...
                        if (data.CouponTypeList && data.CouponTypeList.length > 0) {
                            ...
                            $.each(cacheList, function (index, filed) {
                                $.each(list, function (listIndex, filedList) {
                                    //#bug 2838 会员金矿-大数据，精准营销，选择券的问题
                                    if (filedList.CouponTypeID == filed.CouponTypeID || filedList.SurplusQty <= 0) {
            ...
            that.elems.section.delegate("#submitBtn", "click", function (e) {
                ...
                var me = $(this), dataName = me.data("name");
                me.toggleClass("on");
                //debugger;
                switch (dataName) {
                    case "datebox":
                    ...
                    case "optSelect":
                        ...
                        break;
                    case "numberBoxAndOpt":
                        if (me.hasClass("on")) {
                            //debugger;
                            $("input[data-name='" + me.data("filed") + "']").numberbox({ disabled: false });
                            me.parent().find(".numberbox").removeClass("bgColor");
                            me.parents(".tagPanel").find("." + dataName).show();

                        } else {
                            $("input[data-name='" + me.data("filed") + "']").numberbox({ disabled: true });
                            me.parent().find(".numberbox").addClass("bgColor");
                            me.parents(".tagPanel").find("." + dataName).hide();
                            me.parents(".tagPanel").find(".checkBox." + dataName).removeClass("on");
                        }
                        break;
                }
                        ...
                        if (data.Data.PrizesInfoList && data.Data.PrizesInfoList.length > 0) {
                            $.each(that.elems.PrizesInfoList, function (index, filed) {
                                ...
                                if (domBtn.data("prizestype") == filed.PrizesType) {
                                    ...
                                    //满多少金额的时候
                                    if (filed.AmountLimit) {
                                        data.Data["AmountLimit"] = filed.AmountLimit;
                                        /*$('input[name="AmountLimit"]').numberbox("setValue",filed.AmountLimit)*/
                                    }

                                    if (filed.IsCirculation == 0) {
                                        if ($('.checkBox[data-filed="IsCirculation"]').hasClass('on')) {
                                            $('.checkBox[data-filed="IsCirculation"]').trigger("click");
                                        }
                                    ...
                                    that.elems.prizesCache.push({ type: filed.PrizesType, "list": filed.PrizesDetailList });
        ...
        renderTable: function (data, table) {
            ...
            table.datagrid({
                ...
                columns: [[
                    ...
                    { field: 'ValidityPeriod', title: '有效期', width: 60, align: 'center', resizable: false },
                ...
                if (operationType != "delete") {
                    $.each(pram, function (i, field) {
                        if (field.value != "") {
                            ...
                            if (field.name == "AmountLimit") {
                                AmountLimit = field.value;
                            }
                        }
                    ...
                    case "addCoupon":
                        ...
                        if (dom.length == 0) {
                            ...
                            prams.data["PrizesInfoList"] = [
                                {
                                    "PrizesType": this.args.PrizesType, "PrizesID": this.args.PrizesID,
                                    IsCirculation: $('[data-filed="IsCirculation"].on').length > 0 ? 1 : 0,//是否循环赠送(0=否；1=是)
                                    "IsEnable": true, "AmountLimit": AmountLimit,
                                    "PrizesDetailList": []
                                }
                            ];
                            dom.each(function () {
                    ...
                    case "addMessage":
                        ...
                        if (dom.length == 0) {
                            ...
                            prams.data["ActivityMessageList"] = [
                                {
                                    "MessageID": messageInfo.MessageID, "MessageType": $.trim(messageInfo.MessageType),
                                    /*"TemplateID":dom.data("id"),*/
                            ...
                            $(".templatePanel").each(function (index) {
                                ...
                                if (me.find("textarea").val()) {
                                    ...
                                    if (messageInfo.MessageType == "SMS") {
                                        submit.msg = "短信设置,第" + (me.index()) + "项设置发送信息";
                                    } else if (messageInfo.MessageType == "WeChat") {
                                        submit.msg = "微信设置,第" + (me.index()) + "项设置发送信息";
                                    }
                    ...
                    case "SetPrizes":
                        ...
                        $(".checkBoxPanel").each(function () {
                            ...
                            if ($(this).data("prizesid")) {
                                var IsEnable = me.find('.checkBox.on[data-name="optSelect"]').length > 0 || me.find('.checkBox.on[data-name="numberBoxAndOpt"]').length > 0;
                                prams.data["PrizesInfoList"].push({
                                    "PrizesType": $(this).data("prizestype"),

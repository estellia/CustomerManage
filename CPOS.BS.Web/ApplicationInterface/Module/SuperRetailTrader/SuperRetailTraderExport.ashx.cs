...
namespace JIT.CPOS.BS.Web.ApplicationInterface.Module.SuperRetailTrader
{
    /// <summary>
    /// 导出分销商列表的 Excel
    /// </summary>
    public class SuperRetailTraderExport : JIT.CPOS.BS.Web.PageBase.JITCPOSAjaxHandler
    {
        ...
        protected override void AjaxRequest(HttpContext pContext)
        {
            ...
            if (!String.IsNullOrEmpty(model.SuperRetailTraderFrom))
                pWhereConditions.Add(new EqualsCondition() { FieldName = "a.SuperRetailTraderFrom", Value = model.SuperRetailTraderFrom });

            if (!String.IsNullOrEmpty(model.JoinSatrtTime))
                pWhereConditions.Add(new DirectCondition() { Expression = "a.JoinTime>='" + model.JoinSatrtTime + "'" });

            if (!String.IsNullOrEmpty(model.JoinEndTime))
                pWhereConditions.Add(new DirectCondition() { Expression = "a.JoinTime<='" + model.JoinEndTime + "'" });

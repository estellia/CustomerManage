...
";
        ...
        /// <param name="customerId"></param>
        /// <returns></returns>
        public DataSet GetHeadUnitId(string customerId)
        {
            string sql = "SELECT  * FROM dbo.T_Unit WHERE  customer_id = '" + customerId +
                         "' AND type_id in (select type_id from T_Type WHERE customer_id = '" + customerId +
                         "' AND type_code = '总部')";
            DataSet ds = this.SQLHelper.ExecuteDataset(sql);
            return ds;
        }
...
where b.IsDelete = '0' and customer_id='{0}'", pCustomerID);
		...
		/// <returns></returns>
		public int GetUnitCountByCustomerId(string strCustomerId) {
			string strSql =string.Format(@"SELECT  COUNT(1)
											FROM    dbo.t_unit a
													INNER JOIN dbo.T_Type b ON a.type_id = b.type_id
																			   AND b.type_system_flag = 1
																			   AND b.type_code = '门店'
											WHERE   a.Status = 1
													AND a.customer_id = '{0}'", strCustomerId);

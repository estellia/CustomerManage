...
namespace JIT.CPOS.BS.DataAccess
{
   ...
   ", CurrentUserInfo.CurrentLoggingManager.Customer_Id, CurrentUserInfo.UserID);
            ...
            sql += string.Format(@"SELECT x.StatusType,x.OptionText StatusTypeName,ISNULL(y.StatusCount,0) StatusCount,x.sequence FROM ( 
                  select OptionValue as StatusType,OptionText,sequence From Options where OptionName ='TInOutStatus' and CustomerID='{0}' and isdelete=0               
                  ) x LEFT JOIN (
                  SELECT isnull(a.Field7,-99) StatusType,COUNT(*) StatusCount FROM dbo.T_Inout a 
            ...
            sql += string.Format(@"
                UNION                  
                  ...
                  FROM     ( " + sqlTemp + @" ) aa
                            INNER JOIN T_Inout bb ON aa.order_id = bb.order_id
                  WHERE     bb.Field7 NOT IN ( '600', '700', '800', '900' )
                            AND (bb.sales_unit_id = '' or bb.sales_unit_id is null) ORDER BY sequence
            ", CurrentUserInfo.CurrentLoggingManager.Customer_Id);
        ...
        /// <param name="orderList"></param>
        /// <returns></returns>
        public DataSet GetOrderUnitList(string orderList)
        {
            string sql = string.Format("SELECT * FROM T_Inout WHERE order_no IN ({0})", orderList);
   ...
   ", CurrentUserInfo.CurrentLoggingManager.Customer_Id, CurrentUserInfo.UserID);
        ...
        public DataSet SearchInoutInfo_lj4(OrderSearchInfo orderSearchInfo)
        {
             string orderby = " order by a.order_date desc,a.create_time desc";
            if (!string.IsNullOrEmpty(orderSearchInfo.InoutSort))
            {
            ...
            string sqlTemp = GetSearchPublicSql4(orderSearchInfo, 1);
            #region
            string sql = "";
            //先把需要全表扫描的数据查出来，后面就不需要查了
            //if (orderSearchInfo.path_unit_id != null && orderSearchInfo.path_unit_id.Length > 0)
   ...
   ", CurrentUserInfo.CurrentLoggingManager.Customer_Id, CurrentUserInfo.UserID);
            ...
            sql += "select distinct a.customer_id,a.order_id "
                ...
                //返回的现金，是用的2
                        + @",isnull((SELECT  ISNULL(SUM(Amount),0) FROM dbo.VipAmountDetail	WHERE AmountSourceId=2  AND ObjectId=a.order_id),0) as AmountBack"
               //优惠券抵扣的
                        + @"  , isnull((select z.parValue from couponUse as x inner join Coupon as y on x.couponId =y.couponid
inner join CouponType as z on y.CouponTypeID = z.CouponTypeID where x.orderid = a.order_id),0) couponParValue"
                //会员折扣
                + @", convert(decimal(18,2),  (total_amount-isnull((select top 1 DeliveryAmount from TOrderCustomerDeliveryStrategyMapping x where x.OrderId=a.order_id),0))*(100 - isnull(discount_rate,100)) * 0.01  )  as discountAmount"
                //余额抵扣，积分抵扣（换算成金额），配送费用。
               + @" ,convert(decimal(18,4) ,isnull((select top 1 DeliveryAmount from TOrderCustomerDeliveryStrategyMapping x where x.OrderId=a.order_id),0) ) as  DeliveryAmount
 ,convert(decimal(18,4) ,case when field3 = ''then 0  when field3 is null then 0  end  )as remainSumDis
 ,convert(decimal(18,4),isnull(Pay_Points,0)*convert(decimal(18,2),( select isnull(SettingValue,'0') From CustomerBasicSetting where IsDelete = '0' 
            and CustomerID = a.customer_id 
           and SettingCode = 'IntegralAmountPer'))) as PayPointsDis
"


                        + " From T_Inout a "
                      + " inner join #UnitTmp b "
                      + " on a.order_id = b.order_id "
                      + " left join vip c on a.vip_no = c.vipid "
                      + " where 1=1 "
                      + " and b.row_no > '" + orderSearchInfo.StartRow + "' and b.row_no <= '" + orderSearchInfo.EndRow + "' " + orderby + " drop table #UnitSET;";
        ...
        private string GetSearchPublicSql2(OrderSearchInfo orderSearchInfo, int IsStatus)
        {
            ...
            + " ,order_no  "
            + " ,modify_time "
            + @" from   (select a.order_date,a.create_time,a.order_no,a.order_id ,a.modify_time ,a.purchase_unit_id
                            ,a.sales_unit_id  from  t_inout a ";

            //会员等级查询 添加left join
            if (!string.IsNullOrEmpty(orderSearchInfo.VipCardTypeID))
            {
                sql += @" left join vip v on a.vip_no = v.vipid
                          left join vipcardvipmapping m on v.vipid = m.vipid
                          left join vipcard vc on m.vipcardid = vc.vipcardid";
            }
            ...
            //门店
            if(!string.IsNullOrEmpty(orderSearchInfo.UnitId))
            {
                sql = pService.GetLinkSql(sql, "a.sales_unit_id", orderSearchInfo.UnitId, "=");
            }
            //会员等级
            if (!string.IsNullOrEmpty(orderSearchInfo.VipCardTypeID))
            {
                sql = pService.GetLinkSql(sql, "vc.vipcardtypeid", orderSearchInfo.VipCardTypeID, "=");
            }
            //提货时间
            if(!string.IsNullOrEmpty(orderSearchInfo.ReserveDateBegin))
            {
                sql = pService.GetLinkSql(sql, "a.reserveday", orderSearchInfo.ReserveDateBegin, ">=");//提货开始时间
            }
            if (!string.IsNullOrEmpty(orderSearchInfo.ReserveDateEnd))
            {
                sql = pService.GetLinkSql(sql, "a.reserveday", orderSearchInfo.ReserveDateEnd, "<="); //提货结束时间
            }
            ...
            if (IsStatus == 1)
            {
                if ((!string.IsNullOrEmpty(orderSearchInfo.DeliveryStatus)) && orderSearchInfo.DeliveryStatus != "0")
                {
                        ...
                        //未分配门店
                    {
                        ...
                        var headUnit = unitServer.GetHeadUnitId(orderSearchInfo.customer_id);

                        #endregion

                        sql +=
                            " AND( a.sales_unit_id = '' OR  a.sales_unit_id is null OR a.sales_unit_id = '" + headUnit.Tables[0].Rows[0]["unit_id"] +
                            "') AND a.Field7 NOT IN ('600','700','800','900') ";
                    }
                    #region Bear 把“已发货/已服务/已提货”3个状态合并成“已处理”显示 自定义650 对应Inout3Handler.ashx.cs Line 3230 

                    else if (orderSearchInfo.DeliveryStatus == "650")
                    {
                        sql += " AND a.Field7 IN (620,610,600)";
                    }
                    #endregion
                    else if (orderSearchInfo.DeliveryStatus == "300")
                    {
                        sql += " AND (a.Field7 = '300' OR a.Field1 = '0')";
                    }
                    else
                    {
        ...
        private string GetSearchPublicSql4(OrderSearchInfo orderSearchInfo, int IsStatus)
        {
            string orderby = " order by x.order_date desc,x.create_time DESC,x.order_no desc";
            if (!string.IsNullOrEmpty(orderSearchInfo.InoutSort))
            {
            ...
            PublicService pService = new PublicService();
            string sql = " Create Table #UnitTmp (order_id Nvarchar(100),row_no int) ";
            sql += @"insert #UnitTmp select DISTINCT order_id, row_no = row_number() over (" + orderby + ") from (";
        ...
        public DataSet GetInoutInfoById_lj(string order_id)
        {
            ...
            string sql = "select a.order_id "
                ...
                //+ " ,(select top 1 unit_name From t_unit where unit_id = a.carrier_id) carrier_name "
                      ...
                      + " ,(select wh_name From t_warehouse where warehouse_id = a.purchase_warehouse_id) purchase_warehouse_name "
                //配送费,查不到数据就变为0
                      + ",isnull((select top 1 DeliveryAmount From TOrderCustomerDeliveryStrategyMapping TOCDSM WHERE TOCDSM.OrderID = a.Order_ID and isdelete=0 ),0) DeliveryAmount"
   ...
   and b.CouponTypeID =CONVERT(NVARCHAR(200), c.CouponTypeID ) and d.orderId=a.order_id )) as couponAmount"
                ...
                + @", (  select top 1 isnull(Amount,0) as Amount from VipAmountDetail
...
WHERE     1=1) as t";
        ...
        public DataSet GetInoutDetailInfoByOrderDate(string order_date_begin, string order_date_end, string strCustomerId)
        {
            ...
            string sql = "select a.order_detail_id "
                      ...
                      + " ,b.prop_4_name"
                      + " ,b.prop_5_name"
                      + " ,c.discount_rate as order_discount_rate"
                      + " ,a.Field1 "
                        + " ,a.Field2 "
                        + " ,a.Field3 "
                        + " ,a.Field4 "

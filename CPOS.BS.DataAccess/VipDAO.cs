...
namespace JIT.CPOS.BS.DataAccess
{
    ...
    public partial class VipDAO : Base.BaseCPOSDAO, ICRUDable<VipEntity>, JIT.Utility.DataAccess.IQueryable<VipEntity>
    {
        ...
        {
            string sql = string.Format("exec spMergeVipInfo '{0}','{1}','{2}','{3}'", pCustomerID, pVipID, pPhone, pBindVipid);
            var result = this.SQLHelper.ExecuteScalar(sql);
...
select @ReturnValue", pCustomerID);
        ...
        public DataSet GetVipCouponDataSet(string vipId, decimal totalPayAmount, int usableRange, string objectID, int type,string CustomerId)
        {
            ...
            paras.Add(new SqlParameter() { ParameterName = "@CustomerId", Value = CustomerId });
        ...
        public DataSet GetVipCardLevel(string strVipId,string strCustomerId)
        {
            ...
            string strSql = string.Format(@"
		                                    ...
		                                    c.VipCardTypeID,
                                            a.VipCardID
                                    FROM    dbo.VipCardVipMapping a WITH ( NOLOCK )
                                            INNER JOIN VipCard b WITH ( NOLOCK ) ON a.VipCardID = b.VipCardID
                                            INNER JOIN SysVipCardType c WITH ( NOLOCK ) ON b.VipCardTypeID = c.VipCardTypeID
                                    WHERE   a.CustomerID = '{0}' AND a.VIPID='{1}'  ", strCustomerId, strVipId);
            ds=this.SQLHelper.ExecuteDataset(strSql);
            return ds;
        }
        ...
        public DataSet GetUserRoles(string userName, string customerId, string password)
        {
            ...
            const string sql =
@"SELECT TDA.def_app_id,TDA.Def_App_Code,TR.role_Code from T_User TU  INNER JOIN T_User_Role TUR ON TU.[user_id]=TUR.[user_id]  
INNER JOIN T_Role TR ON TUR.role_id=TR.role_id INNER JOIN T_Def_App TDA ON TR.def_app_id=TDA.def_app_id  
WHERE  TU.customer_id=@CustomerId  AND (TU.user_code=@UserName or  TU.user_telephone = @UserName) AND TU.user_password=@Password";
            return SQLHelper.ExecuteDataset(CommandType.Text, sql, parameters.ToArray());
        }
        #endregion
    }

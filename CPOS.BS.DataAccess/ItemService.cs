...
namespace JIT.CPOS.BS.DataAccess
{
    public class ItemService : Base.BaseCPOSDAO
    {
        ...
        public DataSet SearchList(Hashtable _ht)
        {
            ...
            sql = sql + @"select   a.item_id   ,a.item_category_id 
        ...
        {
            #region
            // add by liangx 2016-09-21 防止SQL注入            
            string sql = @"
            INSERT  INTO T_Item
                    ( item_id ,
                      item_category_id ,
            ...
            var sqlParameters = new[] {
                new SqlParameter("@Item_Id", itemInfo.Item_Id),
                new SqlParameter("@Item_Category_Id", itemInfo.Item_Category_Id==null?"":itemInfo.Item_Category_Id),
                new SqlParameter("@Item_Code", itemInfo.Item_Code),
                new SqlParameter("@Item_Name", itemInfo.Item_Name),
                new SqlParameter("@Item_Name_EN", itemInfo.Item_Name_En),
                new SqlParameter("@Item_Name_Short", itemInfo.Item_Name_Short),
                new SqlParameter("@Pyzjm", itemInfo.Pyzjm),
                new SqlParameter("@Item_Remark", itemInfo.Item_Remark),
                new SqlParameter("@Status", itemInfo.Status),
                new SqlParameter("@Status_Desc", itemInfo.Status_Desc),
                new SqlParameter("@Create_User_Id", itemInfo.Create_User_Id),
                new SqlParameter("@Create_Time", itemInfo.Create_Time),
                new SqlParameter("@Modify_User_Id", itemInfo.Modify_User_Id),
                new SqlParameter("@Modify_Time", itemInfo.Modify_Time),
                new SqlParameter("@ifgifts", itemInfo.ifgifts),
                new SqlParameter("@ifoften", itemInfo.ifoften),
                new SqlParameter("@ifservice", itemInfo.ifservice),
                new SqlParameter("@isGB", itemInfo.isGB),
                new SqlParameter("@Display_Index", itemInfo.display_index),
                new SqlParameter("@Customer_Id", this.CurrentUserInfo.CurrentUser.customer_id.Trim())
            };

            #endregion
            if (pTran != null)
            {
                this.SQLHelper.ExecuteNonQuery((SqlTransaction)pTran, CommandType.Text, sql, sqlParameters);
            }
            else
            {
                this.SQLHelper.ExecuteNonQuery(sql);
     ...
     "') <> 1 then isnull((select avg(c.LowestPrice)	 from StoreItemDailyStatus c where c.SkuID=a.sku_id	and StatusDate>=cast('{0}' as datetime) and StatusDate<cast('{1}' as datetime)   ),a.salesPrice) * dbo.Fn_GetVipDiscountRate('" + userId + "','" + customerId
        ...
        public DataSet GetItemProp2List(string itemId, string propDetailId, string prop1DetailId, int type,string eventId) //1-普通商品 2-团购抢购 3-砍价
        {
            if (string.IsNullOrEmpty(prop1DetailId))
            {
            ...
            {
                string sql = "SELECT DISTINCT a.sku_id skuId,a.sku_prop_id3 prop2DetailId,ISNULL(b.prop_name,a.sku_prop_id3) prop2DetailName,";
                if (type == 1)
                {
                    sql += @"c.Stock,c.SalesCount";
                }
                ...
                if (type == 1)
                {
                    sql = sql + " WHERE a.status = '1' "
                           + " AND a.sku_prop_id1 = '" + propDetailId + "' "
                           + " AND a.sku_prop_id2 = '" + prop1DetailId + "' "
                           + " AND a.item_id = '" + itemId + "'";
                }
                if (type == 2)
                {

    ...
    public partial class StoreBrandMappingDAO : Base.BaseCPOSDAO, ICRUDable<StoreBrandMappingEntity>, IQueryable<StoreBrandMappingEntity>
    {
        ...
        private string GetStoreListByItemSql(string ItemId, string Longitude, string Latitude)
        {
            string sql = "SELECT x.* "
                        ...
                        + " FROM (select * From dbo.t_unit where type_id =(select top 1 type_id from T_Type where customer_id = '" + this.CurrentUserInfo.CurrentUser.customer_id + "' and type_code='门店') and status='1' and customer_id = '" + this.CurrentUserInfo.CurrentUser.customer_id + "' ) a ";

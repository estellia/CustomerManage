...
namespace JIT.CPOS.BS.DataAccess
{
    public class ItemService : Base.BaseCPOSDAO
    {
        ...
        public DataSet GetHotelItemSkuList(string itemId, string userId, string customerId, DateTime beginDate, DateTime endDate)
        {
            ...
            //价格
            //sql += " ,price = a.Price ";
            sql += @"  ,ISNULL( (select avg(c.SourcePrice)	 from StoreItemDailyStatus c  where c.SkuID=a.sku_id	 and StatusDate>=cast('{0}' as datetime) AND StatusDate<cast('{1}' as datetime)  ),price) as price";

            sql += " ,VipLevelName =  case when  dbo.Fn_GetVipDiscountRate('" + userId + "','" + customerId + "') <> 1 then dbo.Fn_GetVipLevelName('" + userId + "','" + customerId + "') else  '' end ";
            //会员价
            sql += @" ,salesPrice = case when dbo.Fn_GetVipDiscountRate('" + userId + "','" + customerId +
     "') <> 1 then isnull((select avg(c.LowestPrice)	 from StoreItemDailyStatus c where c.SkuID=a.sku_id	and StatusDate>=cast('{0}' as datetime) AND StatusDate<cast('{1}' as datetime)   ),a.salesPrice) * dbo.Fn_GetVipDiscountRate('" + userId + "','" + customerId
                + "') else isnull((select avg(c.LowestPrice)	 from StoreItemDailyStatus c where c.SkuID=a.sku_id	and StatusDate>=cast('{0}' as datetime) AND StatusDate<cast('{1}' as datetime)   ),a.salesPrice) end ";
        ...
        {
            string sql = "select a.*, b.prop_1_detail_name as Specification, b.prop_2_detail_name as SKUDegree from [vw_item_detail] a left join vw_sku b on a.item_id = b.item_id where a.item_code='" + itemCode + "' and a.[CustomerId] = '" + customerId + "'";
            return this.SQLHelper.ExecuteDataset(sql);
        }
        #endregion
        ...
        {
            string sql = "select a.*, b.prop_1_detail_name as Specification, b.prop_2_detail_name as SKUDegree from [vwAllItemDetail] a left join vw_sku b on a.item_id = b.item_id where a.item_code='" + itemCode + "' and a.[CustomerId] = '" + customerId + "'";
            return this.SQLHelper.ExecuteDataset(sql);
        }
        #endregion
        ...
        public DataSet GetItemHomePageList(string pCustomerId)
        {
            ...
            sbSQL.Append(" where 1=1 and CustomerId='"+pCustomerId+"';"); //查询全部商品
            ...
            sbSQL.Append("select top 4 item_name as ItemDescription,isnull(Price,0) CostPrice ,isnull(SalesPrice,0) SalesPrice,isnull(imageUrl,'') ItemUrl,('aldlinks://product/detail/customerid='+CustomerId+'/itemid='+item_id) as ItemDetailUrl from dbo.vw_item_detail where CustomerId='" + pCustomerId + "';");

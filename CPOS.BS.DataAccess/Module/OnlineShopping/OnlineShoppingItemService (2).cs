...
namespace JIT.CPOS.BS.DataAccess
{
    public class OnlineShoppingItemService : Base.BaseCPOSDAO
    {
        ...
        /// <returns></returns>
        public DataSet GetWelfareItemList(string userId, string itemName, string itemTypeId, int page, int pageSize, bool isKeep, string isExchange, string storeId, string isGroupBy)
        {
            page = page <= 0 ? 1 : page;
            ...
            sql += string.Format(@" 
select * From #tmp a where 1=1 and a.displayIndex between '{0}' and '{1}' order by a.displayindex;            
select count(*) count From #tmp a", beginSize, endSize);
        ...
        public string GetWelfareItemListSql(string userId, string itemName, string itemTypeId, bool isKeep, string isExchange, string storeId, string isGroupBy)
        {
            ...
            if (!string.IsNullOrEmpty(itemTypeId))
            {
                sql += string.Format("and exists(select * from fnGetChildCategoryByID('{0}',1) e where a.item_category_id=e.CategoryID)", itemTypeId);
            }
...
WHERE a.item_id = '{1}' ", userId, itemId);
        ...
        public DataSet GetItemSkuList(string itemId)
        {
            ...
            sql += " WHERE a.item_id = '" + itemId + "' order by a.prop1DetailOrder ";
...
where sds.Isdelete=0 and sku.status=1 and sds.StatusDate between '{0}' and '{1}'", beginDate, endDate,userId,customerId);
        ...
        public DataSet GetStoreListByCityName(Dictionary<string, string> pParams)
        {
           ...
           ,convert(nvarchar(18),convert(decimal(18,0),   isnull((select	AVG(LowestPrice) from   StoreItemDailyStatus sids where sids.storeid=unit.unit_id   and StatusDate>=cast('{4}' as datetime) AND StatusDate<cast('{5}' as datetime)),0)*dbo.Fn_GetVipDiscountRate('{7}','{0}')      ) )	 as MinPrice
            ...
            from(
	            ...
	            AND a.StatusDate BETWEEN ''{4}'' AND ''{5}''
...
(select convert(decimal(18,0), avg(c.SourcePrice))	 from StoreItemDailyStatus c where c.SkuID=a.SkuId	 and StatusDate>=cast('{1}' as datetime) AND StatusDate<cast('{2}' as datetime)  ) as Price,
 ...
 SalesPrice = case when  dbo.Fn_GetVipDiscountRate('{4}','{0}') = 1 then  convert(decimal(18,0), (select avg(c.LowestPrice)	 from StoreItemDailyStatus c where c.SkuID=a.SkuId	and StatusDate>=cast('{1}' as datetime) AND StatusDate<cast('{2}' as datetime)   )	)
              else convert(decimal(18,0), dbo.Fn_GetVipDiscountRate('{4}','{0}')*(select avg(c.LowestPrice)	 from StoreItemDailyStatus c where c.SkuID=a.SkuId	and StatusDate>=cast('{1}' as datetime) AND StatusDate<cast('{2}' as datetime)   ))	 end,

...
namespace JIT.CPOS.BS.DataAccess
{
    /// <summary>
    /// 数据访问：  
    /// 表TOrderCustomerDeliveryStrategyMapping的数据访问类     
    /// 1.实现ICRUDable接口
    /// 2.实现IQueryable接口
    /// 3.实现Load方法
    /// </summary>
    public partial class TOrderCustomerDeliveryStrategyMappingDAO : Base.BaseCPOSDAO, ICRUDable<TOrderCustomerDeliveryStrategyMappingEntity>, IQueryable<TOrderCustomerDeliveryStrategyMappingEntity>
    {
        #region 构造函数
        /// <summary>
        /// 构造函数 
        /// </summary>
        public TOrderCustomerDeliveryStrategyMappingDAO(LoggingSessionInfo pUserInfo)
            : base(pUserInfo)
        {
        }
        #endregion
        ...
        public void Create(TOrderCustomerDeliveryStrategyMappingEntity pEntity, IDbTransaction pTran)
        {
            ...
            strSql.Append("insert into [TOrderCustomerDeliveryStrategyMapping](");
            strSql.Append("[OrderId],[DSId],[DeliveryAmount],[CreateTime],[CreateBy],[LastUpdateBy],[LastUpdateTime],[IsDelete],[MappingId])");
            ...
            SqlParameter[] parameters = 
            {
					...
					new SqlParameter("@DSId",SqlDbType.UniqueIdentifier),
			...
			parameters[1].Value = pEntity.DSId;
			parameters[2].Value = pEntity.DeliveryAmount;
        ...
        public TOrderCustomerDeliveryStrategyMappingEntity GetByID(object pID)
        {
            ...
            sql.AppendFormat("select * from [TOrderCustomerDeliveryStrategyMapping] where MappingId='{0}'  and isdelete=0 ", id.ToString());
            //读取数据
            TOrderCustomerDeliveryStrategyMappingEntity m = null;
        ...
        public TOrderCustomerDeliveryStrategyMappingEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [TOrderCustomerDeliveryStrategyMapping] where 1=1  and isdelete=0");
            //读取数据
            List<TOrderCustomerDeliveryStrategyMappingEntity> list = new List<TOrderCustomerDeliveryStrategyMappingEntity>();
            using (SqlDataReader rdr = this.SQLHelper.ExecuteReader(sql.ToString()))
            {
                while (rdr.Read())
                {
                    TOrderCustomerDeliveryStrategyMappingEntity m;
                    this.Load(rdr, out m);
                    list.Add(m);
                }
            }
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(TOrderCustomerDeliveryStrategyMappingEntity pEntity , IDbTransaction pTran,bool pIsUpdateNullField)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
            ...
            //组织参数化SQL
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [TOrderCustomerDeliveryStrategyMapping] set ");
                        if (pIsUpdateNullField || pEntity.OrderId!=null)
                strSql.Append( "[OrderId]=@OrderId,");
            if (pIsUpdateNullField || pEntity.DSId!=null)
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [TOrderCustomerDeliveryStrategyMapping] set  isdelete=1 where MappingId=@MappingId;");
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran) 
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [TOrderCustomerDeliveryStrategyMapping] set  isdelete=1 where MappingId in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;   
            if (pTran == null)
        ...
        public PagedQueryResult<TOrderCustomerDeliveryStrategyMappingEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            ...
            pagedSql.AppendFormat(") as ___rn,* from [TOrderCustomerDeliveryStrategyMapping] where 1=1  and isdelete=0 ");
            //总记录数SQL
            totalCountSql.AppendFormat("select count(1) from [TOrderCustomerDeliveryStrategyMapping] where 1=1  and isdelete=0 ");
            //过滤条件
            if (pWhereConditions != null)
            {
                foreach (var item in pWhereConditions)
            ...
            //执行语句并返回结果
            PagedQueryResult<TOrderCustomerDeliveryStrategyMappingEntity> result = new PagedQueryResult<TOrderCustomerDeliveryStrategyMappingEntity>();
            List<TOrderCustomerDeliveryStrategyMappingEntity> list = new List<TOrderCustomerDeliveryStrategyMappingEntity>();
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(TOrderCustomerDeliveryStrategyMappingEntity pQueryEntity)
        { 
            //获取非空属性数量
            ...
            if (pQueryEntity.DSId!=null)
                lstWhereCondition.Add(new EqualsCondition() { FieldName = "DSId", Value = pQueryEntity.DSId });
        ...
        protected void Load(IDataReader pReader, out TOrderCustomerDeliveryStrategyMappingEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new TOrderCustomerDeliveryStrategyMappingEntity();
			...
			if (pReader["DSId"] != DBNull.Value)
			{
				pInstance.DSId =  (Guid)pReader["DSId"];
			}

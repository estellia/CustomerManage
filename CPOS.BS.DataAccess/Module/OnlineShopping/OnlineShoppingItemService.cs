...
namespace JIT.CPOS.BS.DataAccess
{
    public class OnlineShoppingItemService : Base.BaseCPOSDAO
    {
        ...
        public string GetWelfareItemListSql(string userId, string itemName, string itemTypeId, bool isKeep, string isExchange, string storeId, string isGroupBy, string channelId, int socialSalesType, string strSortName, string strSort, int intVirtual, int isStore = 0)
        {
            ...
            int flag = 0;  //是否是营销分组(0=不是；1=是）
            //获取ItemCategoryID的营销分组数量(flag > 0，则ItemCategory表示为营销分组；flag = 0,则ItemCategory表示商品类别)
            if (!string.IsNullOrEmpty(itemTypeId))
            {
                string sqlTemp = "select Count(*) from Itemcategorymapping where ItemCategoryId = '" + itemTypeId + "'";
                flag = Convert.ToInt32(this.SQLHelper.ExecuteScalar(sqlTemp));
            }
            ...
            sql += @" LEFT JOIN (SELECT  B.ItemId ,B.Price ,B.SalesPrice,B.Qty,a.EventId,a.EventTypeId  
                            FROM    [dbo].[PanicbuyingEvent] A  
                            INNER JOIN (SELECT *                                       
                                        FROM   ( SELECT    p.* , ROW_NUMBER() OVER (partition BY p.ItemId ORDER BY p.SalesPrice ASC ) num
                                                 FROM      dbo.PanicbuyingEventItemMapping p  WITH(NOLOCK) WHERE P.IsDelete=0
                                                   ) A
                                         WHERE  A.num = 1) B ON A.EventId = B.EventId   WHERE   CustomerID = '" + this.CurrentUserInfo.CurrentLoggingManager.Customer_Id + "' AND GETDATE() BETWEEN A.BeginTime AND a.EndTime AND b.IsDelete=0) D ON D.ItemId = A.item_id";
            ...
            //ItemTypeId表示的是营销分组
            if (flag > 0)
                sql += " INNER JOIN Itemcategorymapping ICM on a.Item_Id = ICM.ItemId AND ICM.ItemCategoryId = '" + itemTypeId + "' AND ICM.ISDELETE = 0";
            ...
            //itemTypeId存在且是表示商品类别
            if (!string.IsNullOrEmpty(itemTypeId) && flag == 0)
            {
...
WHERE a.item_id = '{1}' ", userId, itemId);
        ...
        {
            string sql = "SELECT x.prop1DetailId,x.prop1DetailName,MAX(skuId) skuId,sum(x.Stock) Stock,sum(x.SalesCount) SalesCount FROM ( SELECT DISTINCT a.sku_id skuId,a.sku_prop_id1 prop1DetailId,ISNULL(b.prop_name,'') prop1DetailName,isnull(b.display_index,0) display_index,c.Stock,c.SalesCount FROM dbo.T_Sku a "
                        + " LEFT JOIN dbo.T_Prop b "
                        + " ON(a.sku_prop_id1 = b.prop_id AND b.status = '1')"
...
where order_id='{0}' ", pOrderID);
        ...
        #region GetUseBalance
        public decimal GetUseBalance(string pOrderID)
        {
            StringBuilder sqlStr = new StringBuilder();
            sqlStr.AppendFormat(@"
SELECT field3 FROM T_Inout 
where order_id='{0}' ", pOrderID);

           string temp=  this.SQLHelper.ExecuteScalar(sqlStr.ToString()).ToString();
            string  useBalance=temp=="" ?"0":temp;
            return Convert.ToDecimal(useBalance);
        }
        #endregion

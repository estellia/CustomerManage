...
namespace JIT.CPOS.BS.DataAccess
{
    ...
    public partial class GLServicePersonInfoDAO : Base.BaseCPOSDAO, ICRUDable<GLServicePersonInfoEntity>, IQueryable<GLServicePersonInfoEntity>
    {
        ...
        /// 构造函数 
        /// </summary>
        public GLServicePersonInfoDAO(LoggingSessionInfo pUserInfo)
            : base(pUserInfo)
        {
        }
        #endregion
        ...
        public void Create(GLServicePersonInfoEntity pEntity, IDbTransaction pTran)
        {
            ...
            strSql.Append("insert into cpos_demo.dbo.[GL_ServicePersonInfo](");
            strSql.Append("[Name],[Mobile],[Picture],[Star],[OrderCount],[CustomerID],[CreateBy],[CreateTime],[LastUpdateBy],[LastUpdateTime],[IsDelete],[UserID])");
			...
			parameters[2].Value = pEntity.Picture;
			parameters[3].Value = pEntity.Star;
			parameters[4].Value = pEntity.OrderCount;
        ...
        public GLServicePersonInfoEntity GetByID(object pID)
        {
            //参数检查
            if (pID == null)
                return null;
            ...
            sql.AppendFormat("select * from cpos_demo.dbo.[GL_ServicePersonInfo] where UserID='{0}' and IsDelete=0 ", id.ToString());
            //读取数据
            GLServicePersonInfoEntity m = null;
        ...
        /// </summary>
        /// <returns></returns>
        public GLServicePersonInfoEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from cpos_demo.dbo.[GL_ServicePersonInfo] where isdelete=0");
            //读取数据
            List<GLServicePersonInfoEntity> list = new List<GLServicePersonInfoEntity>();
            using (SqlDataReader rdr = this.SQLHelper.ExecuteReader(sql.ToString()))
            {
                while (rdr.Read())
                {
                    GLServicePersonInfoEntity m;
                    this.Load(rdr, out m);
                    list.Add(m);
                }
            }
        ...
        public void Update(GLServicePersonInfoEntity pEntity , bool pIsUpdateNullField, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update cpos_demo.dbo.[GL_ServicePersonInfo] set ");
            if (pIsUpdateNullField || pEntity.Name!=null)
                strSql.Append( "[Name]=@Name,");
            if (pIsUpdateNullField || pEntity.Mobile!=null)
                strSql.Append( "[Mobile]=@Mobile,");
            if (pIsUpdateNullField || pEntity.Picture!=null)
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(GLServicePersonInfoEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update cpos_demo.dbo.[GL_ServicePersonInfo] set LastUpdateTime=@LastUpdateTime,LastUpdateBy=@LastUpdateBy,IsDelete=1 where UserID=@UserID;");
        ...
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(GLServicePersonInfoEntity[] pEntities, IDbTransaction pTran)
        {
            //整理主键值
        ...
        /// </summary>
        /// <param name="pEntities">实体实例数组</param>
        public void Delete(GLServicePersonInfoEntity[] pEntities)
        { 
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran) 
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update cpos_demo.dbo.[GL_ServicePersonInfo] set LastUpdateTime='"+DateTime.Now.ToString()+"',LastUpdateBy='"+CurrentUserInfo.UserID+"',IsDelete=1 where UserID in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;   
            if (pTran == null)
        ...
        /// <returns></returns>
        public GLServicePersonInfoEntity[] Query(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys)
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from cpos_demo.dbo.[GL_ServicePersonInfo] where isdelete=0 ");
        ...
        /// <returns></returns>
        public PagedQueryResult<GLServicePersonInfoEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            //组织SQL
            ...
            pagedSql.AppendFormat(") as ___rn,* from [GLServicePersonInfo] where isdelete=0 ");
            //总记录数SQL
            totalCountSql.AppendFormat("select count(1) from [GLServicePersonInfo] where isdelete=0 ");
            //过滤条件
            if (pWhereConditions != null)
            {
                foreach (var item in pWhereConditions)
            ...
            //执行语句并返回结果
            PagedQueryResult<GLServicePersonInfoEntity> result = new PagedQueryResult<GLServicePersonInfoEntity>();
            List<GLServicePersonInfoEntity> list = new List<GLServicePersonInfoEntity>();
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(GLServicePersonInfoEntity pQueryEntity)
        { 
            //获取非空属性数量
            ...
            if (pQueryEntity.Picture!=null)
                lstWhereCondition.Add(new EqualsCondition() { FieldName = "Picture", Value = pQueryEntity.Picture });
        ...
        protected void Load(SqlDataReader pReader, out GLServicePersonInfoEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new GLServicePersonInfoEntity();
			...
			if (pReader["Picture"] != DBNull.Value)
			{
				pInstance.Picture =  Convert.ToString(pReader["Picture"]);
			}

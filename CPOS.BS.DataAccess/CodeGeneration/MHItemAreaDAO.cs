...
namespace JIT.CPOS.BS.DataAccess
{
    ...
    public partial class MHItemAreaDAO : Base.BaseCPOSDAO, ICRUDable<MHItemAreaEntity>, IQueryable<MHItemAreaEntity>
    {
        ...
        /// 构造函数 
        /// </summary>
        public MHItemAreaDAO(LoggingSessionInfo pUserInfo)
            : base(pUserInfo)
        {
        }
        #endregion
        ...
        public MHItemAreaEntity GetByID(object pID)
        {
            ...
            sql.AppendFormat("select * from [MHItemArea] where ItemAreaId='{0}'  and isdelete=0 ", id.ToString());
            //读取数据
            MHItemAreaEntity m = null;
        ...
        public MHItemAreaEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [MHItemArea] where 1=1  and isdelete=0");
            //读取数据
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(MHItemAreaEntity pEntity , IDbTransaction pTran,bool pIsUpdateNullField)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
            if (!pEntity.ItemAreaId.HasValue)
            {
            ...
            //组织参数化SQL
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [MHItemArea] set ");
                        if (pIsUpdateNullField || pEntity.HomeId!=null)
                strSql.Append( "[HomeId]=@HomeId,");
            if (pIsUpdateNullField || pEntity.IsUrl!=null)
        ...
        public void Delete(MHItemAreaEntity pEntity, IDbTransaction pTran)
        {
            ...
            //执行 
            this.Delete(pEntity.ItemAreaId.Value, pTran);           
        }
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [MHItemArea] set  isdelete=1 where ItemAreaId=@ItemAreaId;");
            SqlParameter[] parameters = new SqlParameter[] 
            { 
                new SqlParameter{ParameterName="@ItemAreaId",SqlDbType=SqlDbType.UniqueIdentifier,Value=pID}
            };
            //执行语句
            int result = 0;
            if (pTran != null)
        ...
        public void Delete(MHItemAreaEntity[] pEntities, IDbTransaction pTran)
        {
            ...
            for (int i = 0; i < pEntities.Length; i++)
            {
                ...
                entityIDs[i] = pEntity.ItemAreaId;
            }
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran) 
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [MHItemArea] set  isdelete=1 where ItemAreaId in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;   
            if (pTran == null)
        ...
        public PagedQueryResult<MHItemAreaEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            ...
            {
                pagedSql.AppendFormat(" [ItemAreaId] desc"); //默认为主键值倒序
            }
            pagedSql.AppendFormat(") as ___rn,* from [MHItemArea] where 1=1  and isdelete=0 ");
            //总记录数SQL
            totalCountSql.AppendFormat("select count(1) from [MHItemArea] where 1=1  and isdelete=0 ");
            //过滤条件
            if (pWhereConditions != null)
            {
                foreach (var item in pWhereConditions)
            ...
            //执行语句并返回结果
            PagedQueryResult<MHItemAreaEntity> result = new PagedQueryResult<MHItemAreaEntity>();
            List<MHItemAreaEntity> list = new List<MHItemAreaEntity>();
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(MHItemAreaEntity pQueryEntity)
        { 
            //获取非空属性数量
            List<EqualsCondition> lstWhereCondition = new List<EqualsCondition>();
            if (pQueryEntity.ItemAreaId!=null)
                lstWhereCondition.Add(new EqualsCondition() { FieldName = "ItemAreaId", Value = pQueryEntity.ItemAreaId });
            if (pQueryEntity.HomeId!=null)
                lstWhereCondition.Add(new EqualsCondition() { FieldName = "HomeId", Value = pQueryEntity.HomeId });
            if (pQueryEntity.IsUrl!=null)
                lstWhereCondition.Add(new EqualsCondition() { FieldName = "IsUrl", Value = pQueryEntity.IsUrl });
        ...
        protected void Load(IDataReader pReader, out MHItemAreaEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new MHItemAreaEntity();
            pInstance.PersistenceHandle = new PersistenceHandle();
            pInstance.PersistenceHandle.Load();

			if (pReader["ItemAreaId"] != DBNull.Value)
			{
				pInstance.ItemAreaId =  (Guid)pReader["ItemAreaId"];
			}
			if (pReader["HomeId"] != DBNull.Value)
			{
				pInstance.HomeId =  (Guid)pReader["HomeId"];
			}
			if (pReader["IsUrl"] != DBNull.Value)
			{
				pInstance.IsUrl =   Convert.ToInt32(pReader["IsUrl"]);
			}

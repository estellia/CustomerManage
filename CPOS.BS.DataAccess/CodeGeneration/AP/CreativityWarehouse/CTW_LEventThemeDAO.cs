...
namespace JIT.CPOS.BS.DataAccess
{
    ...
    public partial class CTW_LEventThemeDAO : Base.BaseCPOSDAO, ICRUDable<CTW_LEventThemeEntity>, IQueryable<CTW_LEventThemeEntity>
    {
        ...
        /// 构造函数 
        /// </summary>
        public CTW_LEventThemeDAO(LoggingSessionInfo pUserInfo)
            : base(pUserInfo)
        {
        ...
        public void Create(CTW_LEventThemeEntity pEntity)
        {
        ...
        public void Create(CTW_LEventThemeEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
            ...
            strSql.Append("[TemplateId],[ThemeName],[ImageId],[H5Url],[H5TemplateId],[CreateTime],[CreateBy],[LastUpdateTime],[LastUpdateBy],[IsDelete],[ThemeId])");
			...
			parameters[1].Value = pEntity.ThemeName;
        ...
        public CTW_LEventThemeEntity GetByID(object pID)
        {
            //参数检查
            if (pID == null)
                return null;
        ...
        /// </summary>
        /// <returns></returns>
        public CTW_LEventThemeEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [T_CTW_LEventTheme] where 1=1  and isdelete=0");
            //读取数据
            List<CTW_LEventThemeEntity> list = new List<CTW_LEventThemeEntity>();
            using (SqlDataReader rdr = this.SQLHelper.ExecuteReader(sql.ToString()))
            {
                while (rdr.Read())
                {
                    CTW_LEventThemeEntity m;
                    this.Load(rdr, out m);
                    list.Add(m);
                }
            }
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(CTW_LEventThemeEntity pEntity , IDbTransaction pTran)
        {
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(CTW_LEventThemeEntity pEntity , IDbTransaction pTran,bool pIsUpdateNullField)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
			...
			parameters[7].Value = pEntity.ThemeId;
        ...
        /// </summary>
        /// <param name="pEntity">实体实例</param>
        public void Update(CTW_LEventThemeEntity pEntity )
        {
        ...
        public void Delete(CTW_LEventThemeEntity pEntity)
        {
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(CTW_LEventThemeEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
        ...
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(CTW_LEventThemeEntity[] pEntities, IDbTransaction pTran)
        {
            //整理主键值
        ...
        /// </summary>
        /// <param name="pEntities">实体实例数组</param>
        public void Delete(CTW_LEventThemeEntity[] pEntities)
        { 
        ...
        /// <returns></returns>
        public CTW_LEventThemeEntity[] Query(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys)
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [T_CTW_LEventTheme] where 1=1  and isdelete=0 ");
        ...
        /// <returns></returns>
        public PagedQueryResult<CTW_LEventThemeEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            //组织SQL
            ...
            //执行语句并返回结果
            PagedQueryResult<CTW_LEventThemeEntity> result = new PagedQueryResult<CTW_LEventThemeEntity>();
            List<CTW_LEventThemeEntity> list = new List<CTW_LEventThemeEntity>();
        ...
        public CTW_LEventThemeEntity[] QueryByEntity(CTW_LEventThemeEntity pQueryEntity, OrderBy[] pOrderBys)
        {
        ...
        /// <param name="pOrderBys">排序组合</param>
        /// <returns>符合条件的实体集</returns>
        public PagedQueryResult<CTW_LEventThemeEntity> PagedQueryByEntity(CTW_LEventThemeEntity pQueryEntity, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            IWhereCondition[] queryWhereCondition = GetWhereConditionByEntity( pQueryEntity);
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(CTW_LEventThemeEntity pQueryEntity)
        { 
            //获取非空属性数量
        ...
        protected void Load(IDataReader pReader, out CTW_LEventThemeEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new CTW_LEventThemeEntity();

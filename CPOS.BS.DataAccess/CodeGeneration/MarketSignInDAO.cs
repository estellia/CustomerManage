...
namespace JIT.CPOS.BS.DataAccess
{
    ...
    public partial class MarketSignInDAO : Base.BaseCPOSDAO, ICRUDable<MarketSignInEntity>, IQueryable<MarketSignInEntity>
    {
        ...
        /// 构造函数 
        /// </summary>
        public MarketSignInDAO(LoggingSessionInfo pUserInfo)
            : base(pUserInfo)
        {
        }
        #endregion
        ...
        public MarketSignInEntity GetByID(object pID)
        {
            //参数检查
            if (pID == null)
                return null;
            ...
            sql.AppendFormat("select * from [MarketSignIn] where SignInID='{0}' and IsDelete=0 ", id.ToString());
            //读取数据
            MarketSignInEntity m = null;
        ...
        /// </summary>
        /// <returns></returns>
        public MarketSignInEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
        ...
        public void Update(MarketSignInEntity pEntity , bool pIsUpdateNullField, IDbTransaction pTran)
        {
            ...
            strSql.Append(" where SignInID=@SignInID ");
            SqlParameter[] parameters = 
            {
					new SqlParameter("@OpenID",SqlDbType.NVarChar),
					new SqlParameter("@VipID",SqlDbType.NVarChar),
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(MarketSignInEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
            ...
            //执行 
            this.Delete(pEntity.SignInID, pTran);           
        }
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [MarketSignIn] set LastUpdateTime=@LastUpdateTime,LastUpdateBy=@LastUpdateBy,IsDelete=1 where SignInID=@SignInID;");
            SqlParameter[] parameters = new SqlParameter[] 
            { 
                new SqlParameter{ParameterName="@LastUpdateTime",SqlDbType=SqlDbType.DateTime,Value=DateTime.Now},
                new SqlParameter{ParameterName="@LastUpdateBy",SqlDbType=SqlDbType.VarChar,Value=Convert.ToString(CurrentUserInfo.UserID)},
                new SqlParameter{ParameterName="@SignInID",SqlDbType=SqlDbType.VarChar,Value=pID}
            };
            //执行语句
            int result = 0;
            if (pTran != null)
        ...
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(MarketSignInEntity[] pEntities, IDbTransaction pTran)
        {
            //整理主键值
            ...
            for (int i = 0; i < pEntities.Length; i++)
            {
                ...
                if (item == null)
                    throw new ArgumentNullException("pEntity");
                if (item.SignInID==null)
                {
                    throw new ArgumentException("执行删除时,实体的主键属性值不能为null.");
                }
                entityIDs[i] = item.SignInID;
            }
        ...
        /// </summary>
        /// <param name="pEntities">实体实例数组</param>
        public void Delete(MarketSignInEntity[] pEntities)
        { 
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran) 
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [MarketSignIn] set LastUpdateTime='"+DateTime.Now.ToString()+"',LastUpdateBy='"+CurrentUserInfo.UserID+"',IsDelete=1 where SignInID in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;   
            if (pTran == null)
        ...
        /// <returns></returns>
        public MarketSignInEntity[] Query(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys)
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [MarketSignIn] where 1=1  and isdelete=0 ");
        ...
        /// <returns></returns>
        public PagedQueryResult<MarketSignInEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            //组织SQL
            ...
            //执行语句并返回结果
            PagedQueryResult<MarketSignInEntity> result = new PagedQueryResult<MarketSignInEntity>();
            List<MarketSignInEntity> list = new List<MarketSignInEntity>();
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(MarketSignInEntity pQueryEntity)
        { 
            //获取非空属性数量
            List<EqualsCondition> lstWhereCondition = new List<EqualsCondition>();
            if (pQueryEntity.SignInID!=null)
        ...
        protected void Load(SqlDataReader pReader, out MarketSignInEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new MarketSignInEntity();
            pInstance.PersistenceHandle = new PersistenceHandle();
            pInstance.PersistenceHandle.Load();

			if (pReader["SignInID"] != DBNull.Value)
			{
				pInstance.SignInID =  Convert.ToString(pReader["SignInID"]);
			}

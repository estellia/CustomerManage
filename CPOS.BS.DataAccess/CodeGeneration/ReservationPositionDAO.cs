...
namespace JIT.CPOS.BS.DataAccess
{
    /// <summary>
    /// 数据访问：  
    /// 表ReservationPosition的数据访问类     
    /// 1.实现ICRUDable接口
    /// 2.实现IQueryable接口
    /// 3.实现Load方法
    /// </summary>
    public partial class ReservationPositionDAO : Base.BaseCPOSDAO, ICRUDable<ReservationPositionEntity>, IQueryable<ReservationPositionEntity>
    {
        #region 构造函数
        /// <summary>
        /// 构造函数 
        /// </summary>
        public ReservationPositionDAO(LoggingSessionInfo pUserInfo)
            : base(pUserInfo)
        {
        }
        #endregion
        ...
        public void Create(ReservationPositionEntity pEntity, IDbTransaction pTran)
        {
            ...
            strSql.Append("insert into [ReservationPosition](");
            strSql.Append("[ReservationStoreID],[ItemTitle],[Remark],[CreateTime],[CreateBy],[LastUpdateTime],[LastUpdateBy],[IsDelete],[ReservationPositionID])");
            strSql.Append(" values (");
            strSql.Append("@ReservationStoreID,@ItemTitle,@Remark,@CreateTime,@CreateBy,@LastUpdateTime,@LastUpdateBy,@IsDelete,@ReservationPositionID)");            

			string pkString = pEntity.ReservationPositionID.ToString();
			...
			parameters[1].Value = pEntity.ItemTitle;
            ...
            pEntity.ReservationPositionID = Convert.ToInt32(pkString);
        }
        ...
        public ReservationPositionEntity GetByID(object pID)
        {
            ...
            sql.AppendFormat("select * from [ReservationPosition] where ReservationPositionID='{0}' and IsDelete=0 ", id.ToString());
            //读取数据
            ReservationPositionEntity m = null;
        ...
        public ReservationPositionEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [ReservationPosition] where isdelete=0");
            //读取数据
            List<ReservationPositionEntity> list = new List<ReservationPositionEntity>();
            using (SqlDataReader rdr = this.SQLHelper.ExecuteReader(sql.ToString()))
            {
                while (rdr.Read())
                {
                    ReservationPositionEntity m;
                    this.Load(rdr, out m);
                    list.Add(m);
                }
            }
        ...
        public void Update(ReservationPositionEntity pEntity , bool pIsUpdateNullField, IDbTransaction pTran)
        {
            ...
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
            if (pEntity.ReservationPositionID==null)
            {
            ...
            //组织参数化SQL
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [ReservationPosition] set ");
            ...
            strSql.Append(" where ReservationPositionID=@ReservationPositionID ");
            SqlParameter[] parameters = 
            {
					new SqlParameter("@ReservationStoreID",SqlDbType.Int),
					new SqlParameter("@ItemTitle",SqlDbType.NVarChar),
			...
			parameters[5].Value = pEntity.ReservationPositionID;
        ...
        public void Delete(ReservationPositionEntity pEntity, IDbTransaction pTran)
        {
            ...
            //执行 
            this.Delete(pEntity.ReservationPositionID, pTran);           
        }
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [ReservationPosition] set LastUpdateTime=@LastUpdateTime,LastUpdateBy=@LastUpdateBy,IsDelete=1 where ReservationPositionID=@ReservationPositionID;");
            SqlParameter[] parameters = new SqlParameter[] 
            { 
                new SqlParameter{ParameterName="@LastUpdateTime",SqlDbType=SqlDbType.DateTime,Value=DateTime.Now},
                new SqlParameter{ParameterName="@LastUpdateBy",SqlDbType=SqlDbType.VarChar,Value=Convert.ToString(CurrentUserInfo.UserID)},
                new SqlParameter{ParameterName="@ReservationPositionID",SqlDbType=SqlDbType.VarChar,Value=pID}
            };
            //执行语句
            int result = 0;
            if (pTran != null)
        ...
        public void Delete(ReservationPositionEntity[] pEntities, IDbTransaction pTran)
        {
            ...
            for (int i = 0; i < pEntities.Length; i++)
            {
                ...
                if (item == null)
                    throw new ArgumentNullException("pEntity");
                if (item.ReservationPositionID==null)
                {
                    throw new ArgumentException("执行删除时,实体的主键属性值不能为null.");
                }
                entityIDs[i] = item.ReservationPositionID;
            }
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran) 
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [ReservationPosition] set LastUpdateTime='"+DateTime.Now.ToString()+"',LastUpdateBy='"+CurrentUserInfo.UserID+"',IsDelete=1 where ReservationPositionID in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;   
            if (pTran == null)
        ...
        public PagedQueryResult<ReservationPositionEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            ...
            {
                pagedSql.AppendFormat(" [ReservationPositionID] desc"); //默认为主键值倒序
            }
            pagedSql.AppendFormat(") as ___rn,* from [ReservationPosition] where isdelete=0 ");
            //总记录数SQL
            totalCountSql.AppendFormat("select count(1) from [ReservationPosition] where isdelete=0 ");
            //过滤条件
            if (pWhereConditions != null)
            {
                foreach (var item in pWhereConditions)
            ...
            //执行语句并返回结果
            PagedQueryResult<ReservationPositionEntity> result = new PagedQueryResult<ReservationPositionEntity>();
            List<ReservationPositionEntity> list = new List<ReservationPositionEntity>();
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(ReservationPositionEntity pQueryEntity)
        { 
            //获取非空属性数量
        ...
        protected void Load(SqlDataReader pReader, out ReservationPositionEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new ReservationPositionEntity();

...
namespace JIT.CPOS.BS.DataAccess
{
    ...
    public partial class VipOrderSubRunObjectMappingDAO : Base.BaseCPOSDAO, ICRUDable<VipOrderSubRunObjectMappingEntity>, IQueryable<VipOrderSubRunObjectMappingEntity>
    {
        ...
        /// 构造函数 
        /// </summary>
        public VipOrderSubRunObjectMappingDAO(LoggingSessionInfo pUserInfo)
            : base(pUserInfo)
        {
        }
        #endregion
        ...
        public void Create(VipOrderSubRunObjectMappingEntity pEntity, IDbTransaction pTran)
        {
            ...
            strSql.Append("insert into [VipOrderSubRunObjectMapping](");
            strSql.Append("[VipId],[SubRunObjectId],[SubRunObjectValue],[CustomerId],[CreateBy],[CreateTime],[LastUpdateBy],[LastUpdateTime],[IsDelete],[MappingId])");
			...
			parameters[1].Value = pEntity.SubRunObjectId;
			parameters[2].Value = pEntity.SubRunObjectValue;
        ...
        public VipOrderSubRunObjectMappingEntity GetByID(object pID)
        {
            ...
            sql.AppendFormat("select * from [VipOrderSubRunObjectMapping] where MappingId='{0}'  and isdelete=0 ", id.ToString());
            //读取数据
            VipOrderSubRunObjectMappingEntity m = null;
        ...
        public VipOrderSubRunObjectMappingEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [VipOrderSubRunObjectMapping] where 1=1  and isdelete=0");
            //读取数据
            List<VipOrderSubRunObjectMappingEntity> list = new List<VipOrderSubRunObjectMappingEntity>();
            using (SqlDataReader rdr = this.SQLHelper.ExecuteReader(sql.ToString()))
            {
                while (rdr.Read())
                {
                    VipOrderSubRunObjectMappingEntity m;
                    this.Load(rdr, out m);
                    list.Add(m);
                }
            }
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(VipOrderSubRunObjectMappingEntity pEntity , IDbTransaction pTran,bool pIsUpdateNullField)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
            ...
            //组织参数化SQL
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [VipOrderSubRunObjectMapping] set ");
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [VipOrderSubRunObjectMapping] set  isdelete=1 where MappingId=@MappingId;");
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran) 
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [VipOrderSubRunObjectMapping] set  isdelete=1 where MappingId in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;   
            if (pTran == null)
        ...
        public PagedQueryResult<VipOrderSubRunObjectMappingEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            ...
            pagedSql.AppendFormat(") as ___rn,* from [VipOrderSubRunObjectMapping] where 1=1  and isdelete=0 ");
            //总记录数SQL
            totalCountSql.AppendFormat("select count(1) from [VipOrderSubRunObjectMapping] where 1=1  and isdelete=0 ");
            //过滤条件
            if (pWhereConditions != null)
            {
                foreach (var item in pWhereConditions)
            ...
            //执行语句并返回结果
            PagedQueryResult<VipOrderSubRunObjectMappingEntity> result = new PagedQueryResult<VipOrderSubRunObjectMappingEntity>();
            List<VipOrderSubRunObjectMappingEntity> list = new List<VipOrderSubRunObjectMappingEntity>();
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(VipOrderSubRunObjectMappingEntity pQueryEntity)
        { 
            //获取非空属性数量
        ...
        protected void Load(IDataReader pReader, out VipOrderSubRunObjectMappingEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new VipOrderSubRunObjectMappingEntity();

...
namespace JIT.CPOS.BS.DataAccess
{
    ...
    public partial class SetoffToolsDAO : Base.BaseCPOSDAO, ICRUDable<SetoffToolsEntity>, IQueryable<SetoffToolsEntity>
    {
        ...
        /// 构造函数 
        /// </summary>
        public SetoffToolsDAO(LoggingSessionInfo pUserInfo)
            : base(pUserInfo)
        {
        }
        #endregion
        ...
        public void Create(SetoffToolsEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
            ...
            strSql.Append("insert into [SetoffTools](");
            strSql.Append("[SetoffEventID],[ToolType],[ObjectId],[Status],[CustomerId],[CreateTime],[CreateBy],[LastUpdateTime],[LastUpdateBy],[IsDelete],[SetoffToolID])");
            strSql.Append(" values (");
            strSql.Append("@SetoffEventID,@ToolType,@ObjectId,@Status,@CustomerId,@CreateTime,@CreateBy,@LastUpdateTime,@LastUpdateBy,@IsDelete,@SetoffToolID)");            

			Guid? pkGuid;
			if (pEntity.SetoffToolID == null)
				pkGuid = Guid.NewGuid();
			else
				pkGuid = pEntity.SetoffToolID;
			...
			parameters[1].Value = pEntity.ToolType;
        ...
        public SetoffToolsEntity GetByID(object pID)
        {
            //参数检查
            if (pID == null)
                return null;
            ...
            sql.AppendFormat("select * from [SetoffTools] where SetoffToolID='{0}'  and isdelete=0 ", id.ToString());
            //读取数据
            SetoffToolsEntity m = null;
        ...
        /// </summary>
        /// <returns></returns>
        public SetoffToolsEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [SetoffTools] where 1=1  and isdelete=0");
            //读取数据
            List<SetoffToolsEntity> list = new List<SetoffToolsEntity>();
            using (SqlDataReader rdr = this.SQLHelper.ExecuteReader(sql.ToString()))
            {
                while (rdr.Read())
                {
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(SetoffToolsEntity pEntity , IDbTransaction pTran,bool pIsUpdateNullField)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
            if (!pEntity.SetoffToolID.HasValue)
            {
            ...
            //组织参数化SQL
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [SetoffTools] set ");
            ...
            strSql.Append(" where SetoffToolID=@SetoffToolID ");
            SqlParameter[] parameters = 
            {
					new SqlParameter("@SetoffEventID",SqlDbType.UniqueIdentifier),
					new SqlParameter("@ToolType",SqlDbType.VarChar),
			...
			parameters[7].Value = pEntity.SetoffToolID;
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(SetoffToolsEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
            ...
            //执行 
            this.Delete(pEntity.SetoffToolID.Value, pTran);           
        }
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [SetoffTools] set  isdelete=1 where SetoffToolID=@SetoffToolID;");
            SqlParameter[] parameters = new SqlParameter[] 
            { 
                new SqlParameter{ParameterName="@SetoffToolID",SqlDbType=SqlDbType.UniqueIdentifier,Value=pID}
            };
            //执行语句
            int result = 0;
            if (pTran != null)
        ...
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(SetoffToolsEntity[] pEntities, IDbTransaction pTran)
        {
            //整理主键值
            ...
            for (int i = 0; i < pEntities.Length; i++)
            {
                ...
                entityIDs[i] = pEntity.SetoffToolID;
            }
        ...
        /// </summary>
        /// <param name="pEntities">实体实例数组</param>
        public void Delete(SetoffToolsEntity[] pEntities)
        { 
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran) 
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [SetoffTools] set  isdelete=1 where SetoffToolID in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;   
            if (pTran == null)
        ...
        /// <returns></returns>
        public SetoffToolsEntity[] Query(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys)
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [SetoffTools] where 1=1  and isdelete=0 ");
        ...
        /// <returns></returns>
        public PagedQueryResult<SetoffToolsEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            //组织SQL
            ...
            {
                pagedSql.AppendFormat(" [SetoffToolID] desc"); //默认为主键值倒序
            }
            pagedSql.AppendFormat(") as ___rn,* from [SetoffTools] where 1=1  and isdelete=0 ");
            //总记录数SQL
            totalCountSql.AppendFormat("select count(1) from [SetoffTools] where 1=1  and isdelete=0 ");
            //过滤条件
            if (pWhereConditions != null)
            {
                foreach (var item in pWhereConditions)
            ...
            //执行语句并返回结果
            PagedQueryResult<SetoffToolsEntity> result = new PagedQueryResult<SetoffToolsEntity>();
            List<SetoffToolsEntity> list = new List<SetoffToolsEntity>();
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(SetoffToolsEntity pQueryEntity)
        { 
            //获取非空属性数量
        ...
        protected void Load(IDataReader pReader, out SetoffToolsEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new SetoffToolsEntity();

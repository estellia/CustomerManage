...
namespace JIT.CPOS.BS.DataAccess
{
    ...
    public partial class MarketTemplateDAO : Base.BaseCPOSDAO, ICRUDable<MarketTemplateEntity>, IQueryable<MarketTemplateEntity>
    {
        ...
        /// 构造函数 
        /// </summary>
        public MarketTemplateDAO(LoggingSessionInfo pUserInfo)
            : base(pUserInfo)
        {
        }
        #endregion
        ...
        public void Create(MarketTemplateEntity pEntity, IDbTransaction pTran)
        {
            ...
            strSql.Append("insert into [MarketTemplate](");
            strSql.Append("[TemplateType],[TemplateDesc],[TemplateContent],[CreateTime],[CreateBy],[LastUpdateBy],[LastUpdateTime],[IsDelete],[TemplateContentAPP],[TemplateContentSMS],[CustomerId],[TemplateID])");
            ...
            {
					new SqlParameter("@TemplateType",SqlDbType.NVarChar),
			...
			parameters[0].Value = pEntity.TemplateType;
			...
			parameters[8].Value = pEntity.TemplateContentAPP;
			parameters[9].Value = pEntity.TemplateContentSMS;
        ...
        public MarketTemplateEntity GetByID(object pID)
        {
            //参数检查
            if (pID == null)
                return null;
            ...
            sql.AppendFormat("select * from [MarketTemplate] where TemplateID='{0}' and IsDelete=0 ", id.ToString());
            //读取数据
            MarketTemplateEntity m = null;
        ...
        /// </summary>
        /// <returns></returns>
        public MarketTemplateEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [MarketTemplate] where isdelete=0");
            //读取数据
            List<MarketTemplateEntity> list = new List<MarketTemplateEntity>();
            using (SqlDataReader rdr = this.SQLHelper.ExecuteReader(sql.ToString()))
            {
                while (rdr.Read())
                {
                    MarketTemplateEntity m;
                    this.Load(rdr, out m);
                    list.Add(m);
                }
            }
        ...
        public void Update(MarketTemplateEntity pEntity , bool pIsUpdateNullField, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [MarketTemplate] set ");
            if (pIsUpdateNullField || pEntity.TemplateType!=null)
			...
			parameters[5].Value = pEntity.TemplateContentAPP;
			parameters[6].Value = pEntity.TemplateContentSMS;
			parameters[7].Value = pEntity.CustomerId;
			parameters[8].Value = pEntity.TemplateID;
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(MarketTemplateEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [MarketTemplate] set LastUpdateTime=@LastUpdateTime,LastUpdateBy=@LastUpdateBy,IsDelete=1 where TemplateID=@TemplateID;");
        ...
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(MarketTemplateEntity[] pEntities, IDbTransaction pTran)
        {
            //整理主键值
        ...
        /// </summary>
        /// <param name="pEntities">实体实例数组</param>
        public void Delete(MarketTemplateEntity[] pEntities)
        { 
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran) 
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [MarketTemplate] set LastUpdateTime='"+DateTime.Now.ToString()+"',LastUpdateBy='"+CurrentUserInfo.UserID+"',IsDelete=1 where TemplateID in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;   
            if (pTran == null)
        ...
        /// <returns></returns>
        public MarketTemplateEntity[] Query(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys)
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [MarketTemplate] where isdelete=0 ");
        ...
        /// <returns></returns>
        public PagedQueryResult<MarketTemplateEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            //组织SQL
            ...
            pagedSql.AppendFormat(") as ___rn,* from [MarketTemplate] where isdelete=0 ");
            //总记录数SQL
            totalCountSql.AppendFormat("select count(1) from [MarketTemplate] where isdelete=0 ");
            //过滤条件
            if (pWhereConditions != null)
            {
                foreach (var item in pWhereConditions)
            ...
            //执行语句并返回结果
            PagedQueryResult<MarketTemplateEntity> result = new PagedQueryResult<MarketTemplateEntity>();
            List<MarketTemplateEntity> list = new List<MarketTemplateEntity>();
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(MarketTemplateEntity pQueryEntity)
        { 
            //获取非空属性数量
            ...
            if (pQueryEntity.TemplateType!=null)
                lstWhereCondition.Add(new EqualsCondition() { FieldName = "TemplateType", Value = pQueryEntity.TemplateType });
        ...
        protected void Load(SqlDataReader pReader, out MarketTemplateEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new MarketTemplateEntity();
			...
			if (pReader["TemplateType"] != DBNull.Value)
			{
				pInstance.TemplateType =  Convert.ToString(pReader["TemplateType"]);
			}

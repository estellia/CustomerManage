...
namespace JIT.CPOS.BS.DataAccess
{
    ...
    public partial class APPVersionManagerDAO : Base.BaseCPOSDAO, ICRUDable<APPVersionManagerEntity>, IQueryable<APPVersionManagerEntity>
    {
        ...
        /// 构造函数 
        /// </summary>
        public APPVersionManagerDAO(LoggingSessionInfo pUserInfo)
            : base(pUserInfo)
        {
        }
        #endregion
        ...
        public void Create(APPVersionManagerEntity pEntity)
        {
        ...
        public void Create(APPVersionManagerEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
            ...
            strSql.Append("insert into [APPVersionManager](");
        ...
        public APPVersionManagerEntity GetByID(object pID)
        {
            //参数检查
            if (pID == null)
                return null;
            ...
            sql.AppendFormat("select * from [APPVersionManager] where VersionID='{0}'  and isdelete=0 ", id.ToString());
            //读取数据
            APPVersionManagerEntity m = null;
        ...
        /// </summary>
        /// <returns></returns>
        public APPVersionManagerEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [APPVersionManager] where 1=1  and isdelete=0");
            //读取数据
            List<APPVersionManagerEntity> list = new List<APPVersionManagerEntity>();
            using (SqlDataReader rdr = this.SQLHelper.ExecuteReader(sql.ToString()))
            {
                while (rdr.Read())
                {
                    APPVersionManagerEntity m;
                    this.Load(rdr, out m);
                    list.Add(m);
                }
            }
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(APPVersionManagerEntity pEntity , IDbTransaction pTran)
        {
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(APPVersionManagerEntity pEntity , IDbTransaction pTran,bool pIsUpdateNullField)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
            ...
            //组织参数化SQL
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [APPVersionManager] set ");
        ...
        /// </summary>
        /// <param name="pEntity">实体实例</param>
        public void Update(APPVersionManagerEntity pEntity )
        {
        ...
        public void Delete(APPVersionManagerEntity pEntity)
        {
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(APPVersionManagerEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [APPVersionManager] set  isdelete=1 where VersionID=@VersionID;");
        ...
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(APPVersionManagerEntity[] pEntities, IDbTransaction pTran)
        {
            //整理主键值
        ...
        /// </summary>
        /// <param name="pEntities">实体实例数组</param>
        public void Delete(APPVersionManagerEntity[] pEntities)
        { 
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran) 
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [APPVersionManager] set  isdelete=1 where VersionID in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;   
            if (pTran == null)
        ...
        /// <returns></returns>
        public APPVersionManagerEntity[] Query(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys)
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [APPVersionManager] where 1=1  and isdelete=0 ");
        ...
        /// <returns></returns>
        public PagedQueryResult<APPVersionManagerEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            //组织SQL
            ...
            {
                pagedSql.AppendFormat(" [VersionID] desc"); //默认为主键值倒序
            }
            pagedSql.AppendFormat(") as ___rn,* from [APPVersionManager] where 1=1  and isdelete=0 ");
            //总记录数SQL
            totalCountSql.AppendFormat("select count(1) from [APPVersionManager] where 1=1  and isdelete=0 ");
            //过滤条件
            if (pWhereConditions != null)
            {
                foreach (var item in pWhereConditions)
            ...
            //执行语句并返回结果
            PagedQueryResult<APPVersionManagerEntity> result = new PagedQueryResult<APPVersionManagerEntity>();
            List<APPVersionManagerEntity> list = new List<APPVersionManagerEntity>();
        ...
        public APPVersionManagerEntity[] QueryByEntity(APPVersionManagerEntity pQueryEntity, OrderBy[] pOrderBys)
        {
        ...
        /// <param name="pOrderBys">排序组合</param>
        /// <returns>符合条件的实体集</returns>
        public PagedQueryResult<APPVersionManagerEntity> PagedQueryByEntity(APPVersionManagerEntity pQueryEntity, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            IWhereCondition[] queryWhereCondition = GetWhereConditionByEntity( pQueryEntity);
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(APPVersionManagerEntity pQueryEntity)
        { 
            //获取非空属性数量
        ...
        protected void Load(IDataReader pReader, out APPVersionManagerEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new APPVersionManagerEntity();

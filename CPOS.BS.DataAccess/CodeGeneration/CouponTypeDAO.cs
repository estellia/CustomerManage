...
namespace JIT.CPOS.BS.DataAccess
{
    ...
    public partial class CouponTypeDAO : Base.BaseCPOSDAO, ICRUDable<CouponTypeEntity>, IQueryable<CouponTypeEntity>
    {
        ...
        public List<CouponTypeEntity> GetVirtualItemCouponTypes(string orderId)
        {
            string sql = @"
            SELECT  inout.order_id ,
                    inout.customer_id ,
                    item.item_id ,
                    item.item_name ,
                    inout.vip_no ,
                   
                     ISNULL(virtualItemType.Item_Count, 0)*inoutDetail.order_qty AS Item_Count  ----每个虚拟商品包含的优惠券数量*商品的数量
      ,couponType.*
  ,vip.WeiXinUserId
            FROM    T_Inout inout
                    JOIN T_Inout_Detail inoutDetail ON inout.order_id = inoutDetail.order_id
                    JOIN T_Sku sku ON sku.sku_id = inoutDetail.sku_id
                    JOIN T_Item item ON item.item_id = sku.item_id
                                        AND item.ifservice = 1
                    JOIN T_VirtualItemTypeSetting virtualItemType ON item.item_id = virtualItemType.ItemId
                    JOIN CouponType couponType ON virtualItemType.ObjecetTypeId =  CONVERT(VARCHAR(50),couponType.CouponTypeID)
                inner join vip on vip.vipid=inout.vip_no
            WHERE   inout.order_id = @OrderId   and virtualItemType.isdelete=0";

            SqlParameter[] parameters = new SqlParameter[] {
                new SqlParameter("@OrderId", orderId)
            };

            var queryDataset = SQLHelper.ExecuteDataset(CommandType.Text, sql, parameters);

            List<CouponTypeEntity> lstResult = new List<CouponTypeEntity>();

            if (queryDataset != null && queryDataset.Tables.Count > 0
                && queryDataset.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow itemRow in queryDataset.Tables[0].Rows)
                {
                    var couponType = new CouponTypeEntity();

                    couponType.CustomerId = itemRow["customer_id"].ToString();
                    couponType.CouponTypeID = new Guid(itemRow["CouponTypeID"].ToString());
                    couponType.VipId = itemRow["vip_no"].ToString();
                    couponType.Item_Count = Convert.ToInt32(itemRow["Item_Count"]);
                    couponType.CouponTypeName = itemRow["CouponTypeName"].ToString();

                    if (itemRow["ServiceLife"] != DBNull.Value)
                    {
                        couponType.ServiceLife = Convert.ToInt32(itemRow["ServiceLife"]);
                    }
                    else {
                        couponType.ServiceLife = 0;
                    }
                    if (itemRow["BeginTime"] != DBNull.Value)
                    {
                        couponType.BeginTime = Convert.ToDateTime(itemRow["BeginTime"]);
                    }
                    if (itemRow["EndTime"] != DBNull.Value)
                    {
                        couponType.EndTime = Convert.ToDateTime(itemRow["EndTime"]);
                    }
                    if (itemRow["EndTime"] != DBNull.Value)
                    {
                        couponType.CouponCategory = itemRow["CouponCategory"].ToString();
                    }
                    if (itemRow["WeiXinUserId"] != DBNull.Value)
                    {
                        couponType.WeiXinUserId = itemRow["WeiXinUserId"].ToString();
                    }


                    lstResult.Add(couponType);
                }
            }

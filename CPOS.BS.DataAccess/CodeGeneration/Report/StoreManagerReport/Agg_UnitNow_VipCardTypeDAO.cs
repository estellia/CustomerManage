...
namespace JIT.CPOS.BS.DataAccess
{
    /// <summary>
    /// 数据访问：  
    /// 表Agg_UnitNow_VipCardType的数据访问类     
    /// 1.实现ICRUDable接口
    /// 2.实现IQueryable接口
    /// 3.实现Load方法
    /// </summary>
    public partial class Agg_UnitNow_VipCardTypeDAO : Base.BaseCPOSDAO, ICRUDable<Agg_UnitNow_VipCardTypeEntity>, IQueryable<Agg_UnitNow_VipCardTypeEntity>
    {
        #region 构造函数
        /// <summary>
        /// 构造函数 
        /// </summary>
        public Agg_UnitNow_VipCardTypeDAO(LoggingSessionInfo pUserInfo)
            : base(pUserInfo)
        {
        }
        #endregion
        ...
        public void Create(Agg_UnitNow_VipCardTypeEntity pEntity)
        {
        ...
        public void Create(Agg_UnitNow_VipCardTypeEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
            ...
            strSql.Append("insert into [Agg_UnitNow_VipCardType](");
        ...
        public Agg_UnitNow_VipCardTypeEntity GetByID(object pID)
        {
            //参数检查
            if (pID == null)
                return null;
            ...
            sql.AppendFormat("select * from [Agg_UnitNow_VipCardType] where ID='{0}'  ", id.ToString());
            //读取数据
            Agg_UnitNow_VipCardTypeEntity m = null;
        ...
        /// </summary>
        /// <returns></returns>
        public Agg_UnitNow_VipCardTypeEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [Agg_UnitNow_VipCardType] where 1=1 ");
            //读取数据
            List<Agg_UnitNow_VipCardTypeEntity> list = new List<Agg_UnitNow_VipCardTypeEntity>();
            using (SqlDataReader rdr = this.SQLHelper.ExecuteReader(sql.ToString()))
            {
                while (rdr.Read())
                {
                    Agg_UnitNow_VipCardTypeEntity m;
                    this.Load(rdr, out m);
                    list.Add(m);
                }
            }
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(Agg_UnitNow_VipCardTypeEntity pEntity , IDbTransaction pTran)
        {
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(Agg_UnitNow_VipCardTypeEntity pEntity , IDbTransaction pTran,bool pIsUpdateNullField)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
            ...
            //组织参数化SQL
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [Agg_UnitNow_VipCardType] set ");
        ...
        /// </summary>
        /// <param name="pEntity">实体实例</param>
        public void Update(Agg_UnitNow_VipCardTypeEntity pEntity )
        {
        ...
        public void Delete(Agg_UnitNow_VipCardTypeEntity pEntity)
        {
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(Agg_UnitNow_VipCardTypeEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [Agg_UnitNow_VipCardType] set  where ID=@ID;");
        ...
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(Agg_UnitNow_VipCardTypeEntity[] pEntities, IDbTransaction pTran)
        {
            //整理主键值
        ...
        /// </summary>
        /// <param name="pEntities">实体实例数组</param>
        public void Delete(Agg_UnitNow_VipCardTypeEntity[] pEntities)
        { 
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran) 
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [Agg_UnitNow_VipCardType] set  where ID in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;   
            if (pTran == null)
        ...
        /// <returns></returns>
        public Agg_UnitNow_VipCardTypeEntity[] Query(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys)
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [Agg_UnitNow_VipCardType] where 1=1  ");
        ...
        /// <returns></returns>
        public PagedQueryResult<Agg_UnitNow_VipCardTypeEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            //组织SQL
            ...
            pagedSql.AppendFormat(") as ___rn,* from [Agg_UnitNow_VipCardType] where 1=1  ");
            //总记录数SQL
            totalCountSql.AppendFormat("select count(1) from [Agg_UnitNow_VipCardType] where 1=1  ");
            //过滤条件
            if (pWhereConditions != null)
            {
                foreach (var item in pWhereConditions)
            ...
            //执行语句并返回结果
            PagedQueryResult<Agg_UnitNow_VipCardTypeEntity> result = new PagedQueryResult<Agg_UnitNow_VipCardTypeEntity>();
            List<Agg_UnitNow_VipCardTypeEntity> list = new List<Agg_UnitNow_VipCardTypeEntity>();
        ...
        public Agg_UnitNow_VipCardTypeEntity[] QueryByEntity(Agg_UnitNow_VipCardTypeEntity pQueryEntity, OrderBy[] pOrderBys)
        {
        ...
        /// <param name="pOrderBys">排序组合</param>
        /// <returns>符合条件的实体集</returns>
        public PagedQueryResult<Agg_UnitNow_VipCardTypeEntity> PagedQueryByEntity(Agg_UnitNow_VipCardTypeEntity pQueryEntity, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            IWhereCondition[] queryWhereCondition = GetWhereConditionByEntity( pQueryEntity);
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(Agg_UnitNow_VipCardTypeEntity pQueryEntity)
        { 
            //获取非空属性数量
        ...
        protected void Load(IDataReader pReader, out Agg_UnitNow_VipCardTypeEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new Agg_UnitNow_VipCardTypeEntity();

...
namespace JIT.CPOS.BS.DataAccess
{
    ...
    public partial class LLotteryLogDAO : Base.BaseCPOSDAO, ICRUDable<LLotteryLogEntity>, IQueryable<LLotteryLogEntity>
    {
        ...
        /// 构造函数 
        /// </summary>
        public LLotteryLogDAO(LoggingSessionInfo pUserInfo)
            : base(pUserInfo)
        {
        }
        #endregion
        ...
        public void Create(LLotteryLogEntity pEntity, IDbTransaction pTran)
        {
            ...
            strSql.Append("insert into [LLotteryLog](");
            strSql.Append("[VipId],[EventId],[lotteryCount],[CreateTime],[CreateBy],[LastUpdateBy],[LastUpdateTime],[IsDelete],[LogId])");
			...
			parameters[2].Value = pEntity.LotteryCount;
        ...
        public LLotteryLogEntity GetByID(object pID)
        {
            //参数检查
            if (pID == null)
                return null;
            ...
            sql.AppendFormat("select * from [LLotteryLog] where LogId='{0}' and IsDelete=0 ", id.ToString());
            //读取数据
            LLotteryLogEntity m = null;
        ...
        /// </summary>
        /// <returns></returns>
        public LLotteryLogEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [LLotteryLog] where isdelete=0");
            //读取数据
            List<LLotteryLogEntity> list = new List<LLotteryLogEntity>();
            using (SqlDataReader rdr = this.SQLHelper.ExecuteReader(sql.ToString()))
            {
                while (rdr.Read())
                {
        ...
        public void Update(LLotteryLogEntity pEntity , bool pIsUpdateNullField, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [LLotteryLog] set ");
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(LLotteryLogEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [LLotteryLog] set LastUpdateTime=@LastUpdateTime,LastUpdateBy=@LastUpdateBy,IsDelete=1 where LogId=@LogId;");
        ...
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(LLotteryLogEntity[] pEntities, IDbTransaction pTran)
        {
            //整理主键值
        ...
        /// </summary>
        /// <param name="pEntities">实体实例数组</param>
        public void Delete(LLotteryLogEntity[] pEntities)
        { 
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran) 
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [LLotteryLog] set LastUpdateTime='"+DateTime.Now.ToString()+"',LastUpdateBy='"+CurrentUserInfo.UserID+"',IsDelete=1 where LogId in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;   
            if (pTran == null)
        ...
        /// <returns></returns>
        public LLotteryLogEntity[] Query(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys)
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [LLotteryLog] where isdelete=0 ");
        ...
        /// <returns></returns>
        public PagedQueryResult<LLotteryLogEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            //组织SQL
            ...
            pagedSql.AppendFormat(") as ___rn,* from [LLotteryLog] where isdelete=0 ");
            //总记录数SQL
            totalCountSql.AppendFormat("select count(1) from [LLotteryLog] where isdelete=0 ");
            //过滤条件
            if (pWhereConditions != null)
            {
                foreach (var item in pWhereConditions)
            ...
            //执行语句并返回结果
            PagedQueryResult<LLotteryLogEntity> result = new PagedQueryResult<LLotteryLogEntity>();
            List<LLotteryLogEntity> list = new List<LLotteryLogEntity>();
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(LLotteryLogEntity pQueryEntity)
        { 
            //获取非空属性数量
        ...
        protected void Load(SqlDataReader pReader, out LLotteryLogEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new LLotteryLogEntity();
